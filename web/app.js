(() => {
  ...
  const messages = {
    zh: {
      modelScope: '使用当前已加载模型识别字体，不据此判断设备系统。', unifiedModelScope: '统一字体模型，联合覆盖已训练的 iOS 与 Android 字体，不据此判断设备系统。',
      modelAliasGroups: '共同识别组：{groups}。相同字形不细分别名或地区版本。',
      modelPreviewNotice: '实验候选模型，覆盖 {count} 类字体；范围外字体仍可能误命名，分数不是准确率。', modelPreviewUnknownCount: '实验候选模型；范围外字体仍可能误命名，分数不是准确率。', modelValidationFailed: '尚未通过稳定版验收。', modelPreviewDocs: '查看评测结果与限制', downloadExperimentalModel: '下载实验字体模型',
    ...
    en: {
      modelScope: 'Identify fonts using the currently loaded model. This does not identify the device’s operating system.', unifiedModelScope: 'One unified font model covers its trained iOS and Android font families. This does not identify the device’s operating system.',
      modelAliasGroups: 'Shared recognition groups: {groups}. Identical glyphs do not distinguish aliases or regional variants.',
      modelPreviewNotice: 'Experimental candidate model covering {count} font families. Unsupported fonts may still be misnamed. Scores are not accuracy.', modelPreviewUnknownCount: 'Experimental candidate model. Unsupported fonts may still be misnamed. Scores are not accuracy.', modelValidationFailed: 'Stable validation has not passed.', modelPreviewDocs: 'View evaluation results and limitations', downloadExperimentalModel: 'Download experimental font model',
  ...
  const modelAvailable = () => modelInfo?.available === true && modelInfo.download_url === '/api/models/font/download';

  function renderUpload() {
    $('start').disabled = requestBusy || !file;
    $('api-upload-command').textContent = 'curl -F "file=@./alipay.png" \'http://allenflux.tech:9000\'';
    $('api-put-command').textContent = 'curl -T ./alipay.png \'http://allenflux.tech:9000/upload/alipay.png\'';
  }

  function setBusy(value) { requestBusy = value; renderUpload(); }
  ...
  function renderFontModel() {
    ...
    const experimental = modelInfo?.release_tier === 'experimental';
    const families = Array.isArray(modelInfo?.families)
      ? [...new Set(modelInfo.families.filter(value => typeof value === 'string' && value.trim() && value !== '__unknown__'))] : [];
    $('model-scope').textContent = t(available && modelInfo?.font_mode === 'unified' ? 'unifiedModelScope' : 'modelScope');
    $('model-preview').hidden = !experimental;
    $('model-preview-notice').textContent = [
      t(families.length ? 'modelPreviewNotice' : 'modelPreviewUnknownCount', {count: families.length}),
      modelInfo?.stable_validation_passed === false ? t('modelValidationFailed') : ''
    ].filter(Boolean).join(' ');
    ...
    const pingFangGroup = families.includes('PingFang') && Array.isArray(pingFangNames)
      && pingFangNames.includes('PingFang SC') && pingFangNames.includes('PingFang TC');
    const groups = Object.entries(modelInfo?.font_label_groups || {})
      .filter(([family, names]) => families.includes(family) && !(family === 'PingFang' && pingFangGroup)
        && Array.isArray(names) && names.length > 1 && names.every(name => typeof name === 'string' && name.trim()));
    const notes = [pingFangGroup ? t('modelPingFangGroup') : '', groups.length ? t('modelAliasGroups', {
      groups: groups.map(([family, names]) => `${family} (${names.join(' / ')})`).join(' · ')
    }) : ''].filter(Boolean);
    $('model-label-note').hidden = !available || !notes.length;
    $('model-label-note').textContent = notes.join(' ');
    ...
    const version = available ? modelInfo.version : healthVersion;
    if (version) {
  ...
  async function loadFontModel() {
    ...
    try {
      const info = await api('/api/models/font', {signal: controller.signal});
  ...
  async function health() {
    try {
      ...
      healthVersion = data.model_version || null;
