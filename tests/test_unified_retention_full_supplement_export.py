"""Full-CNN release contracts without real training, model export or holdout data."""
...
from training import export_unified_retention_full_supplement as module
...
def fixture_run(monkeypatch, tmp_path):
    ...
    for name in ('training/train_unified_retention_full_supplement.py', 'training/evaluate_unified_retention_full_supplement.py'):
    ...
    protocol.update(schema='flux-glyph-unified-retention-full-supplement-protocol-v1', architecture=module.ARCHITECTURE,
        objective_variant=trainer.OBJECTIVE_VARIANT, objective=deepcopy(trainer.OBJECTIVE),
        sampling=deepcopy(trainer.SAMPLING), steps=3000, eval_every=500, seed=2026091408,
    ...
    counter.update(rows, [row['family'] not in trainer.core.CORE_FAMILIES for row in rows])
    ...
    selection = {**deepcopy(protocol), 'schema': 'flux-glyph-unified-retention-full-supplement-selection-v1',
