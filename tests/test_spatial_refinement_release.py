...
from scripts import prepare_spatial_refinement_release as release
...
def fixture(tmp_path):
    ...
    selection.update(trial='refinement-low', android_system_guard={**release.NATIVE_MOBILE_ANDROID_GUARD_POLICY, 'actual':20,'passed':True}, android_system_guard_baseline={**release.NATIVE_MOBILE_ANDROID_GUARD_POLICY, 'actual':27,'passed':True})
    ...
    parity = {'schema': release.PARITY_SCHEMA, 'trial': 'refinement-low', 'passed': True,
        'calibration_promotion_allowed': True, 'promotion_allowed': False,
    ...
    report['source_version_summaries'] = {'candidate': {
        'trial_id': 'refinement-low', 'intended_release_version': release.VERSION,
...
def test_incomplete_or_mismatched_evidence_cannot_promote(tmp_path, kind):
    ...
    if kind in ('guard','trial','cal'):
        ...
        elif kind == 'trial': value['trial'] = 'refinement-high'
