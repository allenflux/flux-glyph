...
from training import cache_unified_unknown_supplement as module
...
def fixture(monkeypatch, tmp_path, state_sha='b'*64):
    ...
    preparation = {'schema': 'flux-glyph-unified-unknown-supplement-freeze-v1', 'families': families,
    ...
    rows = []
    for start, count, family in [(0, 1, 'WenQuanYi Zen Hei'), (1, 2, 'Zhuque Fangsong'), (3, 1, 'Zhuque Fangsong')]:
        rows.append({'split': 'train', 'family': '__unknown__', 'target': 24, 'domain': 'android',
...
@pytest.mark.parametrize('fault', ['cal', 'known', 'target', 'domain', 'unverified', 'order', 'count', 'hash', 'nan', 'range'])
def test_only_verified_unknown_train_pixels_in_exact_region_order_are_accepted(monkeypatch, tmp_path, fault):
    data, _, _ = fixture(monkeypatch, tmp_path)
    if fault == 'cal':data['rows'][0]['split'] = 'calibration'
    elif fault == 'known':data['rows'][0]['family'] = data['families'][0]
...
def test_cli_defaults_are_separate_supplement_data_cache_and_frozen_core():
    ...
    assert args.output.parent.name == 'new-unknown-capture-v1'
