...
import pytest
from training import train_unified_retention_distilled as module
...
def test_correct_non_pingfang_mask_kl_direction_and_exact_student_gradient():
    ...
    total,ce,size,kl,mask=module.distilled_losses(student,ratios,targets,sizes,teacher,FAMILIES)
    expected=(targets!=FAMILIES.index('PingFang'))&(teacher.argmax(1)==targets)
...
def test_invalid_distillation_inputs_are_rejected(fault):
    ...
    with pytest.raises(ValueError):module.distilled_losses(student,ratios,targets,sizes,teacher,FAMILIES)
...
def counter_report():
    ...
    for _ in range(2):
        batch=sampler.batch();counter.update(batch,[r['family']!='PingFang' and i%2==0 for i,r in enumerate(batch)])
...
def test_count_report_reconciles_source_family_domain_and_step_without_pf_teacher():
    ...
    assert report['rows']==192 and report['eligible_rows']>0 and report['eligible_family_rows'].get('PingFang',0)==0
