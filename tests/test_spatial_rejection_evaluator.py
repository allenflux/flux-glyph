...
from training import evaluate_spatial_rejection as evaluator
...
def report_fixture(r21_passed=True, r22_passed=True):
    ...
    selection = {'trial': 'rejection-low', 'calibration_promotion_allowed': True, 'promotion_allowed': False,
                 'development_evaluated': False}
    parity = {'trial': 'rejection-low', 'passed': True, 'calibration_promotion_allowed': True,
...
def test_final_preview_requires_both_18_check_comparisons_and_cal53_parity():
    ...
    assert candidate['trial_id'] == 'rejection-low'
    assert candidate['intended_release_version'] == 'r23-unified-font-short-regions-v1-preview'
    assert 'version' not in candidate
    assert report['schema'] == 'flux-glyph-spatial-rejection-development-regression-v1'
