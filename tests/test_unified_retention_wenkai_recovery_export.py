"""Portable export-boundary checks for candidate 20's TRAIN-only WenKai CE weight."""
from copy import deepcopy

import pytest

from training import export_unified_retention_wenkai_recovery as module
from training import train_unified_retention_wenkai_recovery as trainer
...
def selection_fixture():
    ...
    selection.update(schema='flux-glyph-unified-retention-wide-wenkai-recovery-selection-v1',
...
def counts_fixture():
    return {'schema': 'flux-glyph-retention-wenkai-recovery-counts-v1',
        'wenkai_recovery_weighting': deepcopy(WENKAI_RECOVERY_WEIGHTING),
...
def test_builders_preserve_weighting_full_kl_and_raw_inference_contract():
    ...
    assert exported['validation']['kind'] == 'full_cnn_training_wenkai_recovery_calibration_only_at_export'
...
@pytest.mark.parametrize('fault', ('weighting', 'objective', 'inference', 'deployment', 'prior', 'kl',
    'retention', 'promotion'))
...
def test_fixed_replay_validates_recovery_faces_separately_from_native_core():
    result = module.validate_wenkai_recovery_counts(counts_fixture(), selection_fixture())
    assert result['rows'] == 12000
    assert {row['font_face']: row['rows'] for row in result['source_face_rows']} == {
