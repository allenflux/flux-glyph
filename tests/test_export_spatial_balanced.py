...
from training import export_spatial_balanced as export
...
def selection():
    return {'trial':'balanced-low','families':[*(f'f{i}' for i in range(24)),'__unknown__'],
...
def test_module_import_does_not_import_torch(monkeypatch):
    ...
    assert module.PARITY_SCHEMA == 'flux-glyph-spatial-balanced-onnx-parity-v1'
...
def test_exact_spatial_contract_constants():
    ...
    assert export.TRIALS == ('balanced-low','balanced-medium','balanced-high')
    assert export.REPAIR_WEIGHTS == {'balanced-low':.25,'balanced-medium':.5,'balanced-high':1.}
...
def test_loss_trace_reconstructs_original_and_repair_totals():
    ...
    assert export.validate_preflight(pre,'balanced-low') is pre
...
def test_first_passing_grid_selection_is_strict():
    base={'selected':'balanced-medium','selection_policy':export.SELECTION_POLICY,
          'results':{'balanced-low':{'calibration_passed':False},
                     'balanced-medium':{'calibration_passed':True}}}
    assert export._selected_trial(base)=='balanced-medium'
    for changed in (
        {**base,'results':{**base['results'],'balanced-low':{'calibration_passed':True}}},
        {**base,'results':{'balanced-medium':{'calibration_passed':True}}},
        {**base,'selected':'balanced-high'}):
...
def test_parity_names_trial_and_proves_cal53_plus_android_guard():
    ...
    assert p['schema']==export.PARITY_SCHEMA and p['trial']=='balanced-low'
