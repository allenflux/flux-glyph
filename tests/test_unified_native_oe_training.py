...
from training import train_unified_native_oe as trainer
...
def test_only_new_term_is_replay_unknown_oe_and_focus_is_unchanged():
    from training import train_unified_native_short as previous
    from wide_region_network import WideRegionFontClassifier
    torch.set_num_threads(4); torch.manual_seed(56)
    model = WideRegionFontClassifier(25).train(); values = batch()
    before, old_parts = previous.objective(model, values)
    after, parts = trainer.objective(model, values)
    for key, value in old_parts.items():
        torch.testing.assert_close(parts[key], value, atol=1e-7, rtol=1e-7)
    torch.testing.assert_close(after, before + parts['unknown_oe'], atol=1e-7, rtol=1e-7)
    assert parts['unknown_oe_rows'].item() == sum(r['target'] == 24 for r in values['rows'])
    parts['unknown_oe'].backward()
    affected = values['images'].grad.flatten(1).abs().sum(1)
    assert torch.count_nonzero(affected[96:]) == 0
    assert torch.count_nonzero(affected[:96][values['targets'] != 24]) == 0
    assert torch.all(affected[:96][values['targets'] == 24] > 0)
...
def test_completed_cal_pass_still_requires_export_and_both_development_comparisons(monkeypatch, tmp_path):
    ...
    def objective(model, batch):
        ...
        return value, {'focus_teacher_eligible': value.new_tensor(20), 'unknown_oe_rows': value.new_tensor(16)}
    ...
    assert counts['unknown_oe_rows'] == 64
    assert counts['unknown_oe_policy'] == trainer.UNKNOWN_OE_POLICY
