...
from training import export_unified_short_regions_v3 as export
from training import short_region_objective as short


def test_v3_export_accepts_exact_actual_trainer_objective():
    from training.train_unified_short_regions_v3 import OBJECTIVE
    export.validate_v3_objective(OBJECTIVE)
    for key in ('short_loss', 'replay_teacher', 'original_full_correct_teacher_kl_preserved'):
        changed = deepcopy(OBJECTIVE); changed.pop(key)
        with pytest.raises(ValueError, match='V3 weak-long'):
            export.validate_v3_objective(changed)


def teacher_proof_fixture():
    digest = '1' * 64
    parts = {
        'original': {'tiles': 130854, 'unknown_tiles': 8072, 'restored_r21_tiles': 8072},
        'supplement': {'tiles': 3156, 'unknown_tiles': 3156, 'restored_r21_tiles': 0},
        'known': {'tiles': 4820, 'unknown_tiles': 0, 'restored_r21_tiles': 0},
    }
    for row in parts.values():
        row.update({key: digest for key in ('labels_sha256', 'r22_logits_sha256',
                                            'mixed_logits_sha256', 'log_em_ratio_sha256')})
    return {'schema': 'flux-glyph-original-unknown-teacher-restoration-v1',
            'policy': deepcopy(export.TEACHER_RESTORE_POLICY), 'model_inference': False,
            'optimizer_steps': 0, 'calibration_read': False, 'development_read': False,
            'test_read': False, 'partitions': parts, 'bindings': {'/teacher': '2' * 64}}


def test_teacher_proof_requires_exact_reconstruction_scope_hashes_and_frozen_bindings():
    proof = teacher_proof_fixture()
    assert export.validate_teacher_proof(proof, deepcopy(proof), proof['bindings']) is proof
    mutations = (
        lambda value: value['partitions']['original'].__setitem__('restored_r21_tiles', 8071),
        lambda value: value['partitions']['supplement'].__setitem__('restored_r21_tiles', 1),
        lambda value: value['partitions']['known'].__setitem__('unknown_tiles', 1),
        lambda value: value['partitions']['original'].__setitem__('mixed_logits_sha256', 'bad'),
        lambda value: value['policy'].__setitem__('prediction_routing', True),
    )
    for mutate in mutations:
        changed = deepcopy(proof); mutate(changed)
        with pytest.raises(ValueError):
            export.validate_teacher_proof(changed, changed, proof['bindings'])
    reconstructed = deepcopy(proof)
    reconstructed['partitions']['original']['mixed_logits_sha256'] = '3' * 64
    with pytest.raises(ValueError, match='reconstructed'):
        export.validate_teacher_proof(proof, reconstructed, proof['bindings'])
    with pytest.raises(ValueError, match='source closure'):
        export.validate_teacher_proof(proof, deepcopy(proof), {})
...
def counts_fixture():
    ...
    supervised = {name: export.STEPS // 2 for name in families[:-1]}
    return {'optimizer_steps': export.STEPS, 'replay_rows': export.STEPS * 96,
            'short_rows': export.STEPS * 32, 'short_by_family': short_family,
            'short_supervised_rows': export.STEPS * 12,
            'short_supervised_by_family': supervised,
            'short_supervised_by_glyph_count': {'3': export.STEPS * 6, '4': export.STEPS * 6},
            'teacher_eligible_rows': {'original_unknown_teacher_eligible': 1000,
                                      'supplement_unknown_teacher_eligible': 500},
...
def selection_fixture():
    ...
    selection.update({'families': list(short.FAMILIES), 'training_protocol_sha256': 'a' * 64,
        ...
        'training_teacher_proof': teacher_proof_fixture(), 'teacher_cache': '/teacher'})
...
@pytest.mark.parametrize('fault', ['steps', 'replay_total', 'short_total', 'supervised_total',
                                   'supervised_family', 'supervised_length', 'teacher_scope', 'known_family',
...
def test_count_and_source_isolation_mutations_fail(fault):
    ...
    elif fault == 'supervised_total': counts['short_supervised_rows'] -= 1
    elif fault == 'supervised_family': counts['short_supervised_by_family'][families[0]] -= 1
    elif fault == 'supervised_length': counts['short_supervised_by_glyph_count']['3'] -= 1
    elif fault == 'teacher_scope': counts['teacher_eligible_rows']['original_unknown_teacher_eligible'] = export.STEPS * 16
...
def test_raw_metadata_and_parity_are_cal_only_single_cnn_without_teacher_lineage():
    ...
    assert metadata['training']['r21_original_unknown_teacher_training_only'] is True
    assert metadata['training']['training_teacher_policy'] == export.TEACHER_RESTORE_POLICY
