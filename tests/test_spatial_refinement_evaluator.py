...
from training import evaluate_spatial_refinement as evaluator
...
def report_fixture(r21_passed=True, r22_passed=True):
    ...
    selection = {'trial': 'refinement-low', 'calibration_promotion_allowed': True, 'promotion_allowed': False,
                 'development_evaluated': False}
    parity = {'trial': 'refinement-low', 'passed': True, 'calibration_promotion_allowed': True,
...
def test_final_preview_requires_both_18_check_comparisons_and_cal53_parity():
    ...
    candidate = report['source_version_summaries']['candidate']
    assert candidate['trial_id'] == 'refinement-low'
    assert candidate['intended_release_version'] == 'r23-unified-font-short-regions-v1-preview'
    assert 'version' not in candidate
    assert report['schema'] == 'flux-glyph-spatial-refinement-development-regression-v1'
