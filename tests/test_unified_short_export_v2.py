...
from training import export_unified_short_regions_v2 as export
from training import short_region_objective as short


def test_v2_export_accepts_exact_actual_trainer_objective():
    from training.train_unified_short_regions_v2 import OBJECTIVE
    from copy import deepcopy
    export.validate_confidence_contract(OBJECTIVE)
    for key in ('additional_confidence_retention','original_full_correct_teacher_kl_preserved'):
        changed=deepcopy(OBJECTIVE);changed.pop(key)
        with pytest.raises(ValueError,match='confidence retention'):
            export.validate_confidence_contract(changed)
