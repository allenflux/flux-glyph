"""Tests for the quarter-unit Micro TRAIN cross-entropy increment."""
...
def test_exact_smoothed_sum_over_fixed_denominator_and_gradient_scope():
    batch = rows(); micro = FAMILIES.index(MICRO_FAMILY)
    for index in (2, 19, 77):
        batch[index].update(family=MICRO_FAMILY, target=micro)
    targets = torch.tensor([row['target'] for row in batch])
    logits = torch.randn(96, 25, dtype=torch.float64, requires_grad=True)
    actual = micro_recovery_extra_ce(logits, targets, batch, FAMILIES)
    expected = .25 * F.cross_entropy(logits[[2, 19, 77]], targets[[2, 19, 77]],
...
def test_rejects_relabeling_nontrain_or_invalid_tensors(fault):
    batch = rows(); micro = FAMILIES.index(MICRO_FAMILY)
    batch[0].update(family=MICRO_FAMILY, target=micro)
    targets = torch.tensor([row['target'] for row in batch])
    ...
    with pytest.raises(ValueError):
        micro_recovery_extra_ce(logits, targets, batch, FAMILIES)
