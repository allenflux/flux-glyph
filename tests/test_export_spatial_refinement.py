...
from training import export_spatial_refinement as export


def loss_parts(weight=1.):
    return {'unknown_oe':.2,'unknown_oe_weighted':.05,'unknown_oe_rows':16,
        ...
        'repair_system_confusion':.8,'repair_total':.25*1.2+.05*.4+weight*.8}
...
def selection():
    return {'trial':'refinement-low','families':[*(f'f{i}' for i in range(24)),'__unknown__'],
        ...
        'state_after_sha256':'e'*64,'data_manifest_sha256':'f'*64,'repair_confusion_weight':1.,
...
def test_module_import_does_not_import_torch(monkeypatch):
    ...
    assert module.PARITY_SCHEMA == 'flux-glyph-spatial-refinement-onnx-parity-v1'
...
def test_exact_spatial_contract_constants():
    ...
    assert export.TRIALS == ('refinement-low','refinement-medium','refinement-high')
    assert tuple(export.RATES.values()) == (1.2e-4,1.6e-4,2.4e-4)
    assert export.REPAIR_WEIGHTS == {'refinement-low':1.,'refinement-medium':1.,'refinement-high':1.}
...
def test_loss_trace_reconstructs_original_and_repair_totals():
    parts=loss_parts(); assert export.validate_loss_trace(parts,1.) is parts
    ...
    assert export.validate_preflight(pre,'refinement-low') is pre
...
def test_changed_loss_or_row_budget_is_rejected(field,value):
    ...
    with pytest.raises(ValueError): export.validate_loss_trace(parts,1.)


def test_first_passing_grid_selection_is_strict():
    base={'selected':'refinement-medium','selection_policy':export.SELECTION_POLICY,
          'results':{'refinement-low':{'calibration_passed':False},
                     'refinement-medium':{'calibration_passed':True}}}
    assert export._selected_trial(base)=='refinement-medium'
    for changed in (
        {**base,'results':{**base['results'],'refinement-low':{'calibration_passed':True}}},
        {**base,'results':{'refinement-medium':{'calibration_passed':True}}},
        {**base,'selected':'refinement-high'}):
...
def test_parity_names_trial_and_proves_cal53_plus_android_guard():
    ...
    assert p['schema']==export.PARITY_SCHEMA and p['trial']=='refinement-low'
