...
最终固定回归中，系统字体正确命名由 518 增至 527，错误仍为 1；整体正确命名由 1,013 增至 1,028。但 5 个原本正确的原生区域退为待确认，历史样例也有 Noto 的回退，因此未通过预设的全部旧正确结果保留条件。模型仅作为候选保留，线上继续使用 R17。完整取舍、分字体数量、字号与颜色结果见 [回归报告](ios-system-font-results.md)。
