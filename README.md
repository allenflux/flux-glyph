...
后续模型修复在本地进行。加宽网络、字重均衡和类别间距辅助训练分别通过 41、43、45／46 项保留验收。单向教师约束提高了覆盖，但只通过 40／46 项，仍未替换 R21；下一轮保留完整教师约束，只加强已核实文楷训练样本的分类损失。见[容量与字重修复记录](docs/unified-font-capacity-repair.md)。
