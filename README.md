...
**当前仓库发布包为实验版 `r22-unified-font-retention-v1-preview`。** 它已完成本地 MPS 固定 6,000 步训练、原 46 项 retention 验收、18 项重复开发回归、单 ONNX 全量字体／字号一致性与动态批次检查，以及本地 Docker 下载和独立 CLI 交付验证。文楷正确命名为 456／868，微米黑为 209／868；原生 PingFang、SF Pro、Helvetica 锚点分别为 350／362、127／129、52／52，三组误命名均为 0。

CAL 和开发集都已复用，不是盲测。开发集已知正确覆盖提高，整体与 Android 命名精度、未知拒绝率有预设容许范围内的回退。该包仍是实验版，通用稳定验收未通过，TEST 未执行。实际服务版本以 `/api/health` 与 `/api/models/font` 为准。主要结果和限制见[容量与字重修复记录](docs/unified-font-capacity-repair.md)。
...
R22 的 CAL 中，iOS 原生来源命名精度／已知字体正确覆盖率为 99.03%／79.46%，Android 为 90.50%／52.31%。这些是重复使用的校准集结果。部分新类别覆盖仍低，不能承诺截图中的所有手写字体、金额或时间都已改善。
