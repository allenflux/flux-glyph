...
SEED=2026091413
from retention_confidence_floor_loss import unknown_floor_loss, teacher_confidence_floor
...
OBJECTIVE_VARIANT='full_cnn_monotone_true_class_confidence_floor'
DESIGN_CHANGES=['Unknown exposure requires at least .5 probability and no longer penalizes greater unknown confidence.',
    'Replace full distribution KL by a one-sided true-class confidence floor on truth-correct teacher TRAIN rows.']
...
OBJECTIVE.update(unknown_floor_supervision=UNKNOWN_FLOOR_SUPERVISION,
    teacher_confidence_floor_weight=2.,
    teacher_mask='TRAIN same-tile frozen b08 argmax equals true target, including all known classes and unknown',
    teacher_confidence_floor_reduction='Mean eligible relu(log teacher_true_probability - log student_true_probability); differentiable zero if empty',
...
def read_objective_plan(path, acceptance_plan_sha256):
    plan=read(path)
    expected={'schema':'flux-glyph-confidence-floor-plan-v1',
        ...
        'checkpoint_selection':'fixed final step 3000, no intermediate candidate selection',
...
def full_losses(logits,ratios,targets,sizes,teacher_logits,rows,families):
    ...
    require(len(families)==25 and logits.shape==(96,25) and teacher_logits.shape==logits.shape
            ...
            'Confidence-floor tensors must share shape/device and frozen teacher/native targets')
    require(all(bool(torch.isfinite(value).all()) for value in (logits,ratios,sizes,teacher_logits)),
            'Nonfinite confidence-floor inputs')
    ...
    preservation,mask=teacher_confidence_floor(logits,teacher_logits,targets)
    total=ce+.2*size_loss+2*preservation
    require(bool(torch.isfinite(total)),'Nonfinite confidence-floor training loss')
...
class FullSupplementCounts(TruthCorrectCounter):
    ...
    def report(self):
        return {**super().report(),'schema':'flux-glyph-retention-confidence-floor-counts-v1',
...
def validate_training_counts(report,families,steps):
    ...
    require(report.get('schema')=='flux-glyph-retention-confidence-floor-counts-v1'
...
def train(args):
    ...
    paths=[Path(__file__),ROOT/'training/retention_adapter_network.py',ROOT/'training/train_unified_retention_core.py',
        ...
        ROOT/'src/flux_glyph/unified_font.py',ROOT/'src/flux_glyph/region_font.py',ROOT/'training/evaluate_unified_retention_confidence_floor.py',ROOT/'training/retention_supplement_sampler.py',ROOT/'training/retention_source_balanced_sampler.py',ROOT/'training/retention_paired_known_sampler.py',ROOT/'training/train_unified_retention_source_balanced.py',
    ...
    protocol={'schema':'flux-glyph-unified-retention-confidence-floor-protocol-v1','architecture':ARCHITECTURE,'families':families,
    ...
    for step in range(1,STEPS+1):
        ...
        if step%100==0:print(json.dumps({'step':step,'loss':float(loss.detach()),'ce':float(ce.detach()),'teacher_confidence_floor':float(preservation.detach()),'size_loss':float(size_loss.detach()),'seconds':round(time.monotonic()-started,1)}),flush=True)
    ...
    selection={**protocol,'schema':'flux-glyph-unified-retention-confidence-floor-selection-v1','selected':selected,'history':history,
...
def parser():
    ...
    p.add_argument('--objective-plan',type=Path,default=ROOT/'artifacts/unified-font-v3/objective-audit-v1/PLAN.json')
    p.add_argument('--checkpoint',type=Path,default=ROOT/'artifacts/unified-font-v2/run-core-v1/model.pth')
    p.add_argument('--output',type=Path,default=ROOT/'artifacts/unified-font-v3/run-confidence-floor-v1')
