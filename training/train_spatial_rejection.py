...
from spatial_rejection_objective import POLICY as REPAIR_POLICY, objective as repair_objective
from cache_r22_train_teacher import read, write_new
from train_regions import require, sha, state_sha

CONFIGS = {'rejection-low': 8e-5, 'rejection-medium': 8e-5, 'rejection-high': 8e-5}
REPAIR_WEIGHTS = {'rejection-low': .25, 'rejection-medium': .5, 'rejection-high': 1.}
STEPS = 2400
...
OBJECTIVE = {**parent.OBJECTIVE, 'all_parameters_trained': False,
             ...
             'rationale': 'Preserve known classifier rows and coarse spatial weights while learning rejection and true-font discrimination.'}
...
def preflight(args):
    ...
    for trial, rate in CONFIGS.items():
        ...
        planned = {'schema': 'flux-glyph-spatial-rejection-plan-v1', 'trial': trial,
    ...
    write_new(args.output / 'GRID.json', {'schema': 'flux-glyph-spatial-rejection-grid-v1',
...
def calibrate(model, run, protocol, counts, residual_report, state):
    ...
    selection = {**protocol, 'schema': 'flux-glyph-spatial-rejection-selection-v1',
...
def train(args):
    ...
    for trial, rate in CONFIGS.items():
        ...
        protocol = {**plan, 'schema': 'flux-glyph-spatial-rejection-training-freeze-v1',
        ...
        if allowed:
            selected = trial
            break
    write_new(args.output / 'GRID_RESULT.json', {'schema': 'flux-glyph-spatial-rejection-grid-result-v1',
