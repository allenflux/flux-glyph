#!/usr/bin/env python3
"""Export the native-unknown-supplement residual CNN after complete original CAL parity."""
...
def validate(run, data):
    ...
    require(selection.get('schema') == 'flux-glyph-unified-retention-supplement-selection-v1'
    ...
    require(protocol.get('schema') == 'flux-glyph-unified-retention-supplement-protocol-v1'
    ...
    expected = {'architecture': ARCHITECTURE, 'objective_variant': trainer.OBJECTIVE_VARIANT,
        ...
        'weight_decay': 1e-4, 'gradient_clip_norm': 5., 'training_device': 'cpu', 'seed': 2026091407,
    ...
    required = [ROOT/name for name in ('training/train_unified_retention_supplement.py', 'training/retention_supplement_sampler.py',
        ...
        'training/evaluate_unified_retention_supplement.py', 'src/flux_glyph/unified_font.py', 'src/flux_glyph/region_font.py')]
...
def validate_counts(run, selection):
    ...
    require(sha(run/'SAMPLING.json') == selection['sampling_sha256']
            and sha(run/'TRAINING_COUNTS.json') == selection['training_counts_sha256'], 'Supplement training counts changed')
    sampling = trainer.read(run/'SAMPLING.json'); counts = trainer.read(run/'TRAINING_COUNTS.json')
    slots = {key: trainer.SAMPLING[key]*2000 for key in ('base_known', 'base_unknown', 'supplement_unknown',
        'ios_native_pingfang', 'ios_native_sfpro_helvetica', 'ios_native_original_eight')}
    expected_new = dict.fromkeys(trainer.NEW_SOURCES, 8000)
    require(sampling['schema'] == 'flux-glyph-retention-supplement-sampling-v1'
            and sampling['slots'] == slots and sampling['base']['known_rows'] == 96000
            and sampling['base']['unknown_rows'] == 16000 and sampling['supplement_source_rows'] == expected_new
            and sampling['test_read'] is False and sampling['development_holdout_read'] is False
            and sampling['proposal_rows_discarded'] == 0
            and counts['schema'] == 'flux-glyph-retention-supplement-counts-v1' and counts['steps'] == 2000
            and counts['rows'] == 192000 and counts['objective'] == trainer.OBJECTIVE
            and counts['objective_variant'] == trainer.OBJECTIVE_VARIANT
            and counts['supplement_rows'] == 16000 and counts['original_rows'] == 176000
            and counts['supplement_source_rows'] == expected_new
            and counts['test_read'] is False and counts['development_holdout_read'] is False,
            'Native supplement sampling or objective counts differ')
    family, domain, weighted, eligible, seen, sources = Counter(), Counter(), 0, 0, set(), {}
    for row in counts['source_target_rows']:
        ...
        require(key not in seen and key[0] in ('ios', 'android') and key[2] in selection['families']
                ...
                'Invalid native supplement source accounting')
        if key[2] in trainer.core.CORE_FAMILIES:require(row['base_kl_rows'] == 0, 'Core family received a base KL target')
        ...
        if key[1] in trainer.NEW_SOURCES:
            require(key[0] == 'android' and key[2] == '__unknown__' and row['rows'] == 8000,
                    'New native source was relabelled or its replay quota differs')
        seen.add(key); sources[key] = row['rows']; family[key[2]] += row['rows']; domain[key[0]] += row['rows']
        weighted += row['weight_two_rows']; eligible += row['base_kl_rows']
    actual_sources = {}
    for row in sampling['source_rows']:
        key = (row['domain'], row['source_font_family'], row['target_family'])
        require(key not in actual_sources and type(row['rows']) is int and row['rows'] > 0,
                'Invalid or duplicated actual sampler source counts')
        actual_sources[key] = row['rows']
    per_batch = dict.fromkeys(selection['families'], 2); per_batch['__unknown__'] = 16
    per_batch['PingFang'] += 16
    for name in ('SF Pro', 'Helvetica'):per_batch[name] += 4
    for name in trainer.SAMPLING['original_eight_families']:per_batch[name] += 1
    require(dict(family) == sampling['family_rows'] == {key: value*2000 for key, value in per_batch.items()}
            and dict(domain) == sampling['domain_rows'] and sources == actual_sources
            ...
            and 86000 <= weighted <= 98000
            and all(sources.get(('android', name, '__unknown__')) == 8000 for name in trainer.NEW_SOURCES)
            and sum(value for (domain, source, target), value in sources.items()
                    if target == '__unknown__' and source not in trainer.NEW_SOURCES) == 16000,
            'Actual original/supplement training source totals differ')
    views = sampling['supplement_view_rows']
    require(set(views) <= set(trainer.SAMPLING['supplement_view_weights'])
            and all(type(count) is int and count >= 0 for count in views.values()) and sum(views.values()) == 16000,
            'Supplement view counts do not reconcile')
...
def metadata_for_export(selection, cal, parent_metadata, model_sha256, checkpoint_sha256,
    ...
    return {'schema': 'flux-glyph-unified-region-font-v1', 'algorithm': 'unified-region-cnn64x256-v1',
        ...
        **copy.deepcopy(FIXED_RUNTIME), 'font_label_groups': cal['manifest']['font_label_groups'],
        ...
        'validation': {'kind': 'frozen_encoder_native_unknown_supplement_calibration_only_at_export',
...
def export(args):
    ...
    sources = {str((ROOT/name).resolve()): sha(ROOT/name) for name in
        ('training/export_unified_retention_supplement.py', 'training/train_unified_retention_supplement.py',
