#!/usr/bin/env python3
"""Export the soft-unknown full CNN with its exact TRAIN target distribution and known-only KL provenance."""
...
def validate(run, data):
    ...
    require(selection.get('schema') == 'flux-glyph-unified-retention-soft-unknown-selection-v1'
    ...
    require(protocol.get('schema') == 'flux-glyph-unified-retention-soft-unknown-protocol-v1'
    ...
    expected = {'architecture': ARCHITECTURE, 'objective_variant': trainer.OBJECTIVE_VARIANT,
        ...
        'soft_unknown_supervision': trainer.SOFT_UNKNOWN_SUPERVISION,
        'steps': 3000, 'eval_every': 500, 'batch_size': 96, 'learning_rate': 2e-5, 'minimum_learning_rate': 2e-6,
        'weight_decay': 1e-4, 'gradient_clip_norm': 5., 'seed': 2026091409,
    ...
    required = [ROOT/name for name in ('training/train_unified_retention_soft_unknown.py', 'training/retention_supplement_sampler.py',
        ...
        'training/evaluate_unified_retention_soft_unknown.py', 'src/flux_glyph/unified_font.py', 'src/flux_glyph/region_font.py')]
...
def validate_counts(run, selection):
    ...
    slots = {key: trainer.SAMPLING[key]*3000 for key in ('base_known', 'base_unknown', 'supplement_unknown',
        'ios_native_pingfang', 'ios_native_sfpro_helvetica', 'ios_native_original_eight')}
    expected_new = dict.fromkeys(trainer.NEW_SOURCES, 12000)
    require(sampling['schema'] == 'flux-glyph-retention-supplement-sampling-v1'
            ...
            and sampling['base']['unknown_rows'] == 24000 and sampling['supplement_source_rows'] == expected_new
            and sampling['proposal_rows_discarded'] == 0 and sampling['test_read'] is False
            and sampling['development_holdout_read'] is False
            and counts['family_rows'] == sampling['family_rows'] and counts['domain_rows'] == sampling['domain_rows']
            and counts['supplement_rows'] == 24000 and counts['original_rows'] == 264000
    ...
    require(actual == sources and all(sources.get(('android', source, '__unknown__')) == 12000 for source in trainer.NEW_SOURCES)
            and sum(sampling['view_rows'].values()) == 288000
            and sum(sampling['supplement_view_rows'].values()) == 24000,
            'Full-CNN teacher/source accounting differs from actual sampled examples')


def training_metadata(selection):
    keys = ('objective_variant', 'objective', 'soft_unknown_supervision', 'supplement_data', 'supplement_cache', 'training_data_counts',
...
def metadata_for_export(selection, cal, parent_metadata, model_sha256, checkpoint_sha256,
    ...
    return {'schema': 'flux-glyph-unified-region-font-v1', 'algorithm': 'unified-region-cnn64x256-v1',
        ...
        **copy.deepcopy(FIXED_RUNTIME), 'font_label_groups': cal['manifest']['font_label_groups'],
        ...
        'validation': {'kind': 'full_cnn_soft_unknown_supervision_calibration_only_at_export',
...
def export(args):
    ...
    sources = {str((ROOT/name).resolve()): sha(ROOT/name) for name in
        ('training/export_unified_retention_soft_unknown.py', 'training/train_unified_retention_soft_unknown.py',
    ...
    report = {'schema': SCHEMA, 'passed': True, 'promotion_allowed': True, 'font_model_count': 1, 'encoder_count': 1,
        ...
        'soft_unknown_supervision': copy.deepcopy(selection['soft_unknown_supervision']),
