...
from long_short_supervision import CONTRACT as LONG_SHORT_CONTRACT
...
PARITY_SCHEMA = 'flux-glyph-unified-short-onnx-parity-v3'
...
def validate_v3_objective(objective):
    from train_unified_short_regions_v3 import OBJECTIVE as V3_OBJECTIVE
    require(objective == V3_OBJECTIVE
            and objective.get('short_loss') == LONG_SHORT_CONTRACT
            and objective.get('short_teacher') is None
            and objective.get('short_weight') == .125
            and objective.get('original_full_correct_teacher_kl_preserved') is True
            and objective.get('runtime_changes') is False,
            'V3 weak-long supervision or original-unknown teacher policy differs')
...
def validate_teacher_proof(proof, reconstructed, bindings):
    ...
    require(proof == reconstructed
            ...
            'V3 reconstructed training-teacher proof differs')
    ...
    require(set(parts) == {'original', 'supplement', 'known'}
            ...
            and all(isinstance(parts[name].get(key), str) and len(parts[name][key]) == 64
                    ...
                    ('labels_sha256', 'r22_logits_sha256', 'mixed_logits_sha256',
                     'log_em_ratio_sha256')),
            'V3 teacher partition identity, scope or output digest differs')
    ...
    require(isinstance(proof_bindings, dict) and proof_bindings
            ...
            'V3 teacher reconstruction is absent from the frozen source closure')
    return proof
...
def validate_counts(counts, families):
    ...
    supervised = counts.get('short_supervised_by_family', {})
    require(counts.get('short_supervised_rows') == STEPS * 12 == 28800
            and set(supervised) == set(families[:-1])
            and all(supervised[name] == STEPS // 2 == 1200 for name in families[:-1])
            and counts.get('short_supervised_by_glyph_count') == {
                '3': STEPS * 6, '4': STEPS * 6},
            'V3 known three/four-glyph supervision budget differs')
    ...
    teacher_eligible = counts.get('teacher_eligible_rows', {})
    original_limit = sum(expected_unknown[name] for name in SHORT_UNKNOWN_SOURCES)
    supplement_limit = sum(expected_unknown[name] for name in REPLAY_SAMPLING['supplement_sources'])
    require(set(teacher_eligible) == {'original_unknown_teacher_eligible',
                                     'supplement_unknown_teacher_eligible'}
            and type(teacher_eligible['original_unknown_teacher_eligible']) is int
            and 0 <= teacher_eligible['original_unknown_teacher_eligible'] <= original_limit
            and type(teacher_eligible['supplement_unknown_teacher_eligible']) is int
            and 0 <= teacher_eligible['supplement_unknown_teacher_eligible'] <= supplement_limit,
            'V3 teacher eligibility exceeds its original/supplement unknown source scope')
...
def validate(run, data, plan_path):
    ...
    import train_unified_short_regions_v3 as trainer
    ...
    plan = read(plan_path)
    validate_v3_objective(protocol.get('objective', {}))
    require(selection.get('schema') == 'flux-glyph-unified-short-selection-v3'
            and protocol.get('schema') == 'flux-glyph-unified-short-training-freeze-v3'
            and plan.get('schema') == 'flux-glyph-unified-short-training-plan-v3'
    ...
    require(bindings.get(str(pre_path)) == pre.get('sha256') == sha(pre_path)
            and proof.get('schema') == 'flux-glyph-short-region-gradient-preflight-v3'
            and proof.get('passed') is True and proof.get('steps') == 4
            and proof.get('replay_tiles') == 384 and proof.get('short_tiles') == 128
            and proof.get('short_forwarded_tiles') == 128
            and proof.get('short_supervised_tiles') == 48
            and [int(row.get('parts', {}).get('short_eligible', -1))
                 for row in proof.get('traces', [])] == [0, 0, 24, 24]
    ...
    _, reconstructed_teacher = trainer.load_replay_teacher(Path(protocol.get('teacher_cache', '')), datasets)
...
def training_summary(selection, selection_sha256, checkpoint_sha256):
    ...
    return {'kind': 'R22 weak-long short-region continuation with original-unknown teacher restoration',
        'steps': STEPS,
...
def export(args):
    ...
    sources = {str(path.resolve()): sha(path) for path in [Path(__file__),
        ROOT / 'training/evaluate_unified_short_regions_v3.py', ROOT / 'training/train_unified_short_regions_v3.py',
        ROOT / 'training/long_short_supervision.py', ROOT / 'training/restore_unknown_train_teacher.py',
        ROOT / 'training/train_unified_short_control.py',
    ...
    initializer = torch.load(__import__('train_unified_short_regions_v3').BASE_RUN / 'model.pth',
    ...
    validate_initializer_checkpoint(initializer, selection,
        __import__('train_unified_short_regions_v3').BASE_STATE_SHA,
        __import__('train_unified_short_regions_v3').BASE_SELECTION_SHA)
    ...
    retention = read(Path(__import__('train_unified_short_regions_v3').RETENTION_PLAN))
