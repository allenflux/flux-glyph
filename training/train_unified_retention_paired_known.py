#!/usr/bin/env python3
"""Continue one image CNN with paired known data and unchanged binary-unknown objective."""
...
OBJECTIVE_VARIANT='full_cnn_paired_known_source_balanced_unknown_binary_supervision'
DESIGN_CHANGES=['Add paired known TRAIN rows without changing known class or unknown source totals.',
    'Initialize from the prior source-balanced rank-selected student; keep core cached teacher logits separate.']
OBJECTIVE=previous.OBJECTIVE
full_losses=previous.full_losses
...
class FullSupplementCounts(core.DistillationCounter):
    ...
    def update(self,rows,mask):
        ...
        require(2<=len(extra)<=4 and all(not eligible or row['family']!=core.UNKNOWN for row,eligible in zip(rows,mask)),
                'Unknown target entered teacher KL or new-source cycle differs')
    ...
    def report(self):
        return {**super().report(),'schema':'flux-glyph-retention-paired-known-counts-v1',
...
def validate_training_counts(report,families,steps):
    ...
    require(report.get('schema')=='flux-glyph-retention-paired-known-counts-v1'
    ...
    for row in report['source_target_rows']:
        ...
        require(key not in source_lookup and key[0] in ('ios','android') and key[2] in families
                ...
                and (e==0 or key[2] not in (*core.CORE_FAMILIES,core.UNKNOWN)),'Invalid teacher source population')
...
def load_student_initializer(path,model,families):
    """Whole prior rank-selected student; it is not the core logits teacher."""
    import torch
...
def train(args):
    ...
    paths=[Path(__file__),ROOT/'training/retention_adapter_network.py',ROOT/'training/train_unified_retention_core.py',
        ...
        ROOT/'src/flux_glyph/unified_font.py',ROOT/'src/flux_glyph/region_font.py',ROOT/'training/evaluate_unified_retention_paired_known.py',ROOT/'training/retention_supplement_sampler.py',ROOT/'training/retention_source_balanced_sampler.py',ROOT/'training/retention_paired_known_sampler.py',ROOT/'training/train_unified_retention_source_balanced.py',
    ...
    require(full_before==student_identity['state_sha256']!=base_before
        and initial_groups==student_identity['parameter_groups_sha256'],'Student initializer and cached teacher were conflated')
    ...
    protocol={'schema':'flux-glyph-unified-retention-paired-known-protocol-v1','architecture':ARCHITECTURE,'families':families,
    ...
    old_batch=batch_source(training,cache['train']);new_batch=batch_source(supplement,supplement_cache);known_batch=batch_source(known,known_cache)
    ...
    selection={**protocol,'schema':'flux-glyph-unified-retention-paired-known-selection-v1','selected':selected,'history':history,
...
def parser():
    ...
    p.add_argument('--output',type=Path,default=ROOT/'artifacts/unified-font-v2/run-paired-known-v1')
