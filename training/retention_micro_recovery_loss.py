"""One quarter extra smoothed CE unit for true Micro Hei TRAIN rows."""
from __future__ import annotations

MICRO_FAMILY = 'WenQuanYi Micro Hei'
MICRO_RECOVERY_WEIGHTING = {
    'family': MICRO_FAMILY,
    'training_scope': 'True-label TRAIN rows only',
    'base_known_ce_weight': 1.0,
    'extra_known_ce_weight': 0.25,
    'resulting_known_ce_weight': 1.25,
    'label_smoothing': 0.03,
    'denominator': 96,
    'teacher_eligibility_gated': False,
    'wenkai_recovery_weighting_changed': False,
...
def micro_recovery_extra_ce(logits, targets, rows, families):
    """Return 0.25 times summed smoothed Micro CE divided by 96."""
    import torch
    ...
    valid = (isinstance(logits, torch.Tensor) and logits.ndim == 2
             ...
             and len(set(families)) == 25 and MICRO_FAMILY in families
             and bool(((targets >= 0) & (targets < 25)).all()))
    if not valid:
        raise ValueError('Micro recovery requires one finite canonical 96-row TRAIN batch')
    ...
    for row, target in zip(rows, target_values):
        if not (isinstance(row, dict) and type(row.get('target')) is int
            ...
            raise ValueError('Micro recovery cannot relabel or leave verified TRAIN rows')
    mask = targets == families.index(MICRO_FAMILY)
    if not bool(mask.any()):
        return logits.sum() * 0.0
    return .25 * F.cross_entropy(logits[mask], targets[mask], label_smoothing=.03,
                                 reduction='sum') / 96
