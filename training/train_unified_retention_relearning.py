#!/usr/bin/env python3
"""Train the margin CNN with asymmetric named/unknown teacher preservation."""
...
import train_unified_retention_margin as margin_trial
from retention_dual_teacher_loss import select_teacher
...
from retention_angular_margin_loss import angular_margin_loss,ANGULAR_MARGIN
from retention_asymmetric_teacher_loss import asymmetric_teacher_loss,ASYMMETRIC_PRESERVATION
...
OBJECTIVE_VARIANT='wide_cnn_face_balanced_angular_margin_asymmetric_teacher_relearning'
DESIGN_CHANGES=[
    'Replace only candidate 18 teacher preservation: eligible named rows retain the teacher true-class confidence floor while eligible unknown rows retain full R21 KL.',
    'Keep the b08-to-wide initializer, face-balanced replay, merged TRAIN data, angular auxiliary, base losses, schedule, architecture and runtime fixed.']
OBJECTIVE={key:value for key,value in margin_trial.OBJECTIVE.items() if key not in
    ('teacher_kl_weight','teacher_kl_reduction','teacher_distribution_kl')}
OBJECTIVE.update(teacher_preservation_weight=ASYMMETRIC_PRESERVATION['outer_weight'],
    asymmetric_teacher_preservation=ASYMMETRIC_PRESERVATION,
    ...
    named_teacher_distribution_kl=False,
    unknown_teacher_distribution_kl=True,
...
def read_objective_plan(path, acceptance_plan_sha256, weight_teacher_cache_sha256):
    ...
    expected={'schema':'flux-glyph-wide-named-relearning-plan-v1',
...
def full_losses(logits,ratios,features,family_head_weight,targets,sizes,teacher_logits,rows,families):
    ...
    require(len(families)==25 and logits.shape==(96,25) and teacher_logits.shape==logits.shape
            ...
            'Asymmetric-teacher tensors must share shape/device and frozen teacher/native targets')
    require(all(bool(torch.isfinite(value).all()) for value in (logits,ratios,sizes,teacher_logits)),
            'Nonfinite asymmetric-teacher inputs')
    ...
    if bool(known.any()):
        per_row[known]=torch.nn.functional.cross_entropy(
            logits[known],targets[known],label_smoothing=.03,reduction='none')
    if bool(unknown.any()):
        per_row[unknown]=unknown_floor_loss(logits[unknown],families.index(core.UNKNOWN))
    ce=(per_row*weight).sum()/96
    size_loss=torch.nn.functional.smooth_l1_loss(ratios,sizes)
    preservation,mask=asymmetric_teacher_loss(
        logits,teacher_logits,targets,families.index(core.UNKNOWN))
    require(features.shape==(96,256) and family_head_weight.shape==(25,256)
            and features.dtype==family_head_weight.dtype==logits.dtype
            and features.device==family_head_weight.device==logits.device
            and bool(torch.isfinite(features).all()) and bool(torch.isfinite(family_head_weight).all()),
            'Angular-margin features and family-head weights differ from the current student')
    auxiliary=angular_margin_loss(features,family_head_weight,targets,rows,families)
    total=(ce+.2*size_loss
        +ASYMMETRIC_PRESERVATION['outer_weight']*preservation+auxiliary)
    require(bool(torch.isfinite(total)),'Nonfinite asymmetric-teacher training loss')
...
class FullSupplementCounts(TruthCorrectCounter):
    ...
    def report(self):
        return {**super().report(),'schema':'flux-glyph-retention-named-relearning-counts-v1',
...
def validate_training_counts(report,families,steps):
    ...
    require(report.get('schema')=='flux-glyph-retention-named-relearning-counts-v1'
            ...
            and report.get('asymmetric_teacher_preservation')==ASYMMETRIC_PRESERVATION
...
def train(args):
    ...
    paths=[Path(__file__),ROOT/'training/retention_adapter_network.py',ROOT/'training/train_unified_retention_core.py',
        ...
        ROOT/'training/retention_angular_margin_loss.py',ROOT/'training/train_unified_retention_margin.py',
        ROOT/'training/retention_asymmetric_teacher_loss.py',ROOT/'training/merge_unified_weight_pairs.py',
    ...
    protocol={'schema':'flux-glyph-unified-retention-wide-named-relearning-protocol-v1','architecture':ARCHITECTURE,'families':families,
        ...
        'teacher_distribution_kl':False,
        'teacher_distribution_kl_all_eligible_rows':False,
        'teacher_distribution_kl_scope':OBJECTIVE['teacher_distribution_kl_scope'],
    ...
    for step in range(1,STEPS+1):
        ...
        if step%100==0:print(json.dumps({'step':step,'loss':float(loss.detach()),'ce':float(ce.detach()),'asymmetric_teacher_preservation':float(preservation.detach()),'size_loss':float(size_loss.detach()),'angular_margin_weighted_loss':float(auxiliary.detach()),'seconds':round(time.monotonic()-started,1)}),flush=True)
    ...
    selection={**protocol,'schema':'flux-glyph-unified-retention-wide-named-relearning-selection-v1','selected':selected,'history':history,
...
def parser():
    ...
    p.add_argument('--objective-plan',type=Path,default=ROOT/'artifacts/unified-font-v3/named-relearning-plan-v1/PLAN.json')
    p.add_argument('--checkpoint',type=Path,default=ROOT/'artifacts/unified-font-v2/run-core-v1/model.pth')
    p.add_argument('--output',type=Path,default=ROOT/'artifacts/unified-font-v3/run-wide-named-relearning-v1')
