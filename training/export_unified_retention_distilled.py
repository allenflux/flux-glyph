#!/usr/bin/env python3
"""Export the TRAIN-only distilled retention repair as one student CNN.
...
import train_unified_retention_distilled as retention
...
def objective_contract(selection, protocol):
    ...
    return {'variant': retention.OBJECTIVE_VARIANT, 'objective': retention.OBJECTIVE,
            'source': ROOT/'training/train_unified_retention_distilled.py'}
...
def validate_distillation(report, selection, sampled):
    ...
    require(report['teacher_state_before_sha256'] == report['teacher_state_after_sha256']
            ...
            and all(value <= retention.BATCH_SIZE-19 for value in report['eligible_rows_per_step']),
            'Actual teacher masking does not match frozen teacher identity and supervised TRAIN sampling')
    return report


def export_sources(selection):
    names = ['training/export_unified_retention_distilled.py', 'training/train_unified_retention_distilled.py',
...
def validate(run, data):
    ...
    shared = ('architecture', 'policy', 'fixed_runtime', 'families', 'bindings', 'parent_checkpoint',
              ...
              'objective_variant', 'objective', 'training_model_count', 'teacher_in_deployed_model')
    ...
    required = [data/'MANIFEST.json', data/'train/MANIFEST.json', data/'calibration/MANIFEST.json',
        ...
        ROOT/'training/evaluate_unified_retention_distilled.py', ROOT/'src/flux_glyph/unified_font.py',
    ...
    require(sha(run/'DISTILLATION.json') == selection['distillation_sha256'],
            'Actual TRAIN distillation statistics changed')
