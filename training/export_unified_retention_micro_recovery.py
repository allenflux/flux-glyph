#!/usr/bin/env python3
"""Export one wide CNN trained with a true-Micro TRAIN quarter-CE recovery weight."""
...
def source_entry(entry, bindings):
    require(isinstance(entry, dict) and set(entry) == {'path', 'sha256'}, 'Incomplete export source identity')
    path = (ROOT/entry['path']).resolve()
    require(path.is_file() and sha(path) == entry['sha256'] == bindings.get(str(path)), 'Export source binding differs')
    return path
...
def validate_objective_plan(selection, retention_plan_path):
    ...
    import train_unified_retention_micro_recovery as trainer
...
def validate_fixed_final_selection(selection):
    ...
    require([row['step'] for row in history] == [6000]
            ...
            'WenKai-recovery selection must be the sole fixed final checkpoint')
    return history
...
def validate(run, data):
    ...
    require(selection.get('schema') == 'flux-glyph-unified-retention-wide-micro-recovery-selection-v1'
            and selection.get('architecture') == ARCHITECTURE and selection.get('promotion_allowed') is True
            and not (run/'NO_PROMOTABLE_CHECKPOINT.json').exists(), 'Micro-recovery run has no promotable frozen checkpoint')
    protocol = trainer.read(run/'TRAINING_FREEZE.json')
    require(protocol.get('schema') == 'flux-glyph-unified-retention-wide-micro-recovery-protocol-v1'
            ...
            'WenKai-recovery protocol and selection differ')
    require(all(key not in document for document in (selection, protocol)
            ...
            'Obsolete unknown supervision metadata cannot describe angular-margin training')
    expected = {'architecture': ARCHITECTURE, 'objective_variant': trainer.OBJECTIVE_VARIANT,
        ...
        'angular_margin': trainer.ANGULAR_MARGIN, 'angular_margin_applies_at_inference': False,
        'angular_margin_adds_deployment_parameters': False,
        'wenkai_recovery_weighting': trainer.WENKAI_RECOVERY_WEIGHTING,
        'wenkai_recovery_applies_at_inference': False,
        'wenkai_recovery_adds_deployment_parameters': False,
        'micro_recovery_weighting': trainer.MICRO_RECOVERY_WEIGHTING,
    ...
    validate_angular_margin_provenance(selection)
    validate_wenkai_recovery_provenance(selection)
    validate_micro_recovery_provenance(selection)
    for field in ('initial_parameter_groups_sha256', 'selected_parameter_groups_sha256', 'final_parameter_groups_sha256'):
    ...
    require(base.get('schema') == 'flux-glyph-unified-retention-selection-v1'
            ...
            'Angular-margin base state, classes or source closure differ')
    ...
    require(base['retention_plan']['sha256'] == sha(plan_path), 'Angular-margin retention conditions changed')
    ...
    require(cal['manifest_sha256'] == selection['data_manifest_sha256'] == base['data_manifest_sha256']
            and cal['families'] == selection['families'] == plan['families'], 'Angular-margin data root or class order differs')
    for key in ('parent_metadata', 'parent_checkpoint', 'parent_selection_sha256'):
        require(selection[key] == base[key], 'Angular-margin parent provenance differs')
    parent_metadata = source_entry(selection['parent_metadata'], bindings)
    required = [ROOT/name for name in ('training/train_unified_retention_micro_recovery.py',
        'training/retention_micro_recovery_loss.py', 'training/train_unified_retention_wenkai_recovery.py',
        'training/retention_wenkai_recovery_loss.py', 'training/train_unified_retention_margin.py',
        'training/retention_dual_teacher_loss.py', 'training/retention_confidence_floor_loss.py',
        'training/retention_angular_margin_loss.py', 'training/train_unified_retention_face_balanced.py',
        ...
        'training/evaluate_unified_retention_micro_recovery.py',
    ...
    for split in ('train', 'calibration'):
        ...
        require(part['split'] == split and part['families'] == selection['families']
                and part['root_manifest_sha256'] == cal['manifest_sha256'], 'Angular-margin source partition differs')
        for name in ('metadata', 'array'):
            path = (data/split/part[name]['path']).resolve()
            require(path.parent == data/split and bindings.get(str(path)) == part[name]['sha256'], 'Angular-margin requested data path is unbound')
    ...
    require(all(bindings.get(str(path.resolve())) == sha(path) for path in required), 'Missing exact angular-margin source binding')
    ...
    require(set(selection['baseline_bindings']) == {'BASELINE.json', 'BASELINE_CALIBRATION_DECISIONS.json',
            ...
            'Angular-margin baseline does not reproduce the frozen base cache')
    ...
    for record in history:
        directory = f"checkpoints/step{record['step']:05d}"
        require(set(record['artifacts']) == {'checkpoint', 'outputs', 'decisions', 'metrics'}, 'Incomplete angular-margin step evidence')
        ...
        require(actual == strip_artifacts(record) == trainer.read(paths['metrics']), 'Angular-margin CAL metrics do not reproduce cached outputs')
    for key, name, field in [('outputs', 'CALIBRATION_OUTPUTS.npz', 'calibration_outputs_sha256'),
                            ('decisions', 'CALIBRATION_DECISIONS.json', 'calibration_decisions_sha256')]:
        require(sha(run/name) == selection[field] == selection['selected']['artifacts'][key]['sha256'], 'Selected angular-margin CAL bytes differ')
...
def validate_wenkai_recovery_counts(counts, selection, steps=6000):
    """Validate the extra CE population independently from native-core weighting."""
    import train_unified_retention_micro_recovery as trainer
    from retention_wenkai_recovery_loss import WENKAI_RECOVERY_WEIGHTING as canonical
    require(counts.get('schema') == 'flux-glyph-retention-micro-recovery-counts-v1'
            and counts.get('wenkai_recovery_weighting') == canonical
            and selection.get('wenkai_recovery_weighting') == canonical,
            'Weighted WenKai count contract differs')
    per_step = counts.get('wenkai_recovery_rows_per_step')
    require(isinstance(per_step, list) and len(per_step) == steps
            and all(type(value) is int and value == 2 for value in per_step),
            'Every fixed batch must contain exactly two true WenKai recovery rows')
    rows = counts.get('wenkai_recovery_rows')
    face_rows = {}
    for item in counts.get('wenkai_recovery_source_face_rows', []):
        key = (item.get('domain'), item.get('source_font_family'), item.get('font_face'))
        require(key not in face_rows and type(item.get('rows')) is int and item['rows'] > 0,
                'Invalid weighted WenKai source/face row')
        face_rows[key] = item['rows']
    expected = {('android', 'LXGW WenKai', 'LXGWWenKai-Light'): 4000,
                ('android', 'LXGW WenKai', 'LXGWWenKai-Medium'): 4000,
                ('android', 'LXGW WenKai', 'LXGWWenKai-Regular'): 4000}
    require(steps == 6000 and rows == sum(per_step) == sum(face_rows.values()) == 12000
            and face_rows == expected, 'Fixed replay must weight exactly 4000 true WenKai rows per face')
    native = counts.get('native_core_weighted_family_rows', {})
    require(isinstance(native, dict) and 'LXGW WenKai' not in native,
            'WenKai recovery rows must remain distinct from native-core weighting')
    return {'rows': rows, 'source_face_rows': copy.deepcopy(counts['wenkai_recovery_source_face_rows'])}


def validate_micro_recovery_counts(counts, selection, steps=6000):
    """Validate Micro quarter-CE rows independently of WenKai and native-core weights."""
    import train_unified_retention_micro_recovery as trainer
    from retention_micro_recovery_loss import MICRO_RECOVERY_WEIGHTING as canonical
    require(counts.get('schema') == 'flux-glyph-retention-micro-recovery-counts-v1'
            and counts.get('micro_recovery_weighting') == canonical
            and selection.get('micro_recovery_weighting') == canonical,
            'Weighted Micro count contract differs')
    per_step = counts.get('micro_recovery_rows_per_step')
    require(isinstance(per_step, list) and len(per_step) == steps
            and all(type(value) is int and value == 2 for value in per_step),
            'Every fixed batch must contain exactly two true Micro recovery rows')
    rows = counts.get('micro_recovery_rows')
    source_faces = {}
    for item in counts.get('micro_recovery_source_face_rows', []):
        key = (item.get('domain'), item.get('source_font_family'), item.get('font_face'))
        require(key not in source_faces and type(item.get('rows')) is int and item['rows'] > 0,
                'Invalid weighted Micro source/face row')
        source_faces[key] = item['rows']
    expected = {('android', 'WenQuanYi Micro Hei', 'WenQuanYiMicroHei'): 12000}
    require(steps == 6000 and rows == sum(per_step) == sum(source_faces.values()) == 12000
            and source_faces == expected, 'Fixed replay must weight exactly 12000 true Micro rows')
    native = counts.get('native_core_weighted_family_rows', {})
    require(isinstance(native, dict) and 'WenQuanYi Micro Hei' not in native,
            'Micro recovery rows must remain distinct from native-core weighting')
    require(counts.get('wenkai_recovery_rows') == 12000,
            'Micro recovery cannot change the separately counted WenKai recovery population')
    return {'rows': rows, 'source_face_rows': copy.deepcopy(counts['micro_recovery_source_face_rows'])}
...
def validate_counts(run, selection, data):
    ...
    require(all(key not in document for document in (counts, sampling)
                for key in ('soft_unknown_supervision', 'unknown_binary_supervision')),
            'Obsolete unknown supervision metadata in angular-margin evidence')
    trainer.validate_training_counts(counts, selection['families'], 6000)
    validate_wenkai_recovery_counts(counts, selection, 6000)
    validate_micro_recovery_counts(counts, selection, 6000)
...
def validate_angular_margin_provenance(selection):
    """Require the canonical TRAIN-only auxiliary and unchanged deployed interface."""
    import train_unified_retention_micro_recovery as trainer
    from retention_angular_margin_loss import ANGULAR_MARGIN as canonical
    margin = selection.get('angular_margin')
    require(margin == canonical == trainer.ANGULAR_MARGIN
            and selection.get('objective') == trainer.OBJECTIVE
            and selection['objective'].get('angular_margin') == margin
            and selection.get('objective_variant') == trainer.OBJECTIVE_VARIANT
            and margin.get('inference_rule') is False
            and margin.get('deployed_operator') is False
            and margin.get('unknown_rows_are_anchors') is False
            and selection.get('angular_margin_applies_at_inference') is False
            and selection.get('angular_margin_adds_deployment_parameters') is False,
            'Angular-margin objective is not the canonical TRAIN-only auxiliary')
    require(selection.get('architecture') == ARCHITECTURE
            and selection.get('steps') == selection.get('eval_every') == 6000
            and selection.get('optimizer_steps_executed') == 6000
            and selection.get('selected', {}).get('step') == 6000
            and selection.get('checkpoint_selection') ==
                'Fixed final step 6000; no intermediate CAL inference or checkpoint search'
            and selection.get('model_count') == selection.get('encoder_count') == 1
            and selection.get('platform_routing') is False
            and selection.get('score_merging') is False
            and selection.get('training_inputs') == ['image_tiles'],
            'Angular-margin trial changed fixed selection or the raw model interface')
    return margin


def validate_wenkai_recovery_provenance(selection):
    """Require the canonical TRAIN-only extra CE and unchanged inference contract."""
    import train_unified_retention_micro_recovery as trainer
    from retention_wenkai_recovery_loss import WENKAI_RECOVERY_WEIGHTING as canonical
    weighting = selection.get('wenkai_recovery_weighting')
    objective = selection.get('objective', {})
    require(weighting == canonical == trainer.WENKAI_RECOVERY_WEIGHTING
            and objective == trainer.OBJECTIVE
            and objective.get('wenkai_recovery_weighting') == weighting
            and objective.get('class_prior_weighting') is True
            and objective.get('inference_class_prior_changed') is False
            and selection.get('wenkai_recovery_applies_at_inference') is False
            and selection.get('wenkai_recovery_adds_deployment_parameters') is False
            and selection.get('promotion_allowed') is True
            and selection.get('selected', {}).get('promotion_allowed') is True
            and weighting.get('family') == 'LXGW WenKai'
            and weighting.get('training_scope') == 'True-label TRAIN rows only'
            and weighting.get('base_known_ce_weight') == 1.0
            and weighting.get('extra_known_ce_weight') == 1.0
            and weighting.get('resulting_known_ce_weight') == 2.0
            and weighting.get('label_smoothing') == .03
            and weighting.get('denominator') == 96
            and weighting.get('teacher_eligibility_gated') is False
            and all(weighting.get(key) is False for key in (
                'native_core_weighting_changed', 'angular_margin_weighting_changed',
                'unknown_loss_changed', 'size_loss_changed', 'teacher_preservation_changed',
                'adds_parameters', 'inference_rule', 'deployed_operator'))
            and weighting.get('labels_unchanged') is True
            and objective.get('teacher_distribution_kl') is True,
            'WenKai recovery objective or inference contract differs')
    retention_checks = selection.get('selected', {}).get('retention_checks')
    require(isinstance(retention_checks, list) and len(retention_checks) == 46
            and all(check.get('passed') is True for check in retention_checks),
            'All 46 frozen retention gates must pass before WenKai recovery export')
    forbidden = ('inference_class_weights', 'inference_class_priors', 'inference_logit_bias',
                 'runtime_class_weights', 'runtime_class_priors')
    require(all(key not in selection for key in forbidden)
            and all(key not in objective for key in forbidden),
            'WenKai TRAIN weighting cannot alter inference scores')
    return weighting


def validate_micro_recovery_provenance(selection):
    """Require canonical TRAIN-only Micro quarter CE with no inference effect."""
    import train_unified_retention_micro_recovery as trainer
    from retention_micro_recovery_loss import MICRO_RECOVERY_WEIGHTING as canonical
    weighting = selection.get('micro_recovery_weighting')
    objective = selection.get('objective', {})
    require(weighting == canonical == trainer.MICRO_RECOVERY_WEIGHTING
            and objective == trainer.OBJECTIVE
            and objective.get('micro_recovery_weighting') == weighting
            and selection.get('micro_recovery_applies_at_inference') is False
            and selection.get('micro_recovery_adds_deployment_parameters') is False
            and weighting.get('family') == 'WenQuanYi Micro Hei'
            and weighting.get('training_scope') == 'True-label TRAIN rows only'
            and weighting.get('base_known_ce_weight') == 1.0
            and weighting.get('extra_known_ce_weight') == .25
            and weighting.get('resulting_known_ce_weight') == 1.25
            and weighting.get('label_smoothing') == .03
            and weighting.get('denominator') == 96
            and weighting.get('teacher_eligibility_gated') is False
            and all(weighting.get(key) is False for key in (
                'wenkai_recovery_weighting_changed', 'native_core_weighting_changed',
            ...
            and weighting.get('labels_unchanged') is True
            and objective.get('wenkai_recovery_weighting') == trainer.WENKAI_RECOVERY_WEIGHTING
            and objective.get('teacher_distribution_kl') is True,
            'Micro recovery objective or inference contract differs')
    return weighting
...
def training_metadata(selection):
    ...
    validate_wenkai_recovery_provenance(selection)
    validate_micro_recovery_provenance(selection)
    keys = ('angular_margin', 'angular_margin_applies_at_inference',
        'angular_margin_adds_deployment_parameters', 'wenkai_recovery_weighting',
        'wenkai_recovery_applies_at_inference', 'wenkai_recovery_adds_deployment_parameters',
        'micro_recovery_weighting', 'micro_recovery_applies_at_inference',
        'micro_recovery_adds_deployment_parameters',
        'historical_known_data', 'historical_known_cache', 'known_cache_kind',
    ...
    return {**{key: copy.deepcopy(selection[key]) for key in keys}, 'selected_step': selection['selected']['step'],
        ...
        'class_prior_weighting_training_only': True, 'inference_class_prior_changed': False}
...
def metadata_for_export(selection, cal, parent_metadata, model_sha256, checkpoint_sha256,
    ...
    return {'schema': 'flux-glyph-unified-region-font-v1', 'algorithm': 'unified-region-cnn64x256-v1',
        ...
        **copy.deepcopy(FIXED_RUNTIME), 'font_label_groups': cal['manifest']['font_label_groups'],
        ...
        'validation': {'kind': 'full_cnn_training_micro_recovery_calibration_only_at_export',
...
def build_parity_report(selection, selection_sha256, checkpoint_sha256, model_sha256, metadata_sha256,
    ...
    'angular_margin': copy.deepcopy(selection['angular_margin']),
    'angular_margin_applies_at_inference': selection['angular_margin_applies_at_inference'],
    'angular_margin_adds_deployment_parameters': selection['angular_margin_adds_deployment_parameters'],
    'wenkai_recovery_weighting': copy.deepcopy(selection['wenkai_recovery_weighting']),
    'wenkai_recovery_applies_at_inference': selection['wenkai_recovery_applies_at_inference'],
    'wenkai_recovery_adds_deployment_parameters': selection['wenkai_recovery_adds_deployment_parameters'],
    'micro_recovery_weighting': copy.deepcopy(selection['micro_recovery_weighting']),
    'micro_recovery_applies_at_inference': selection['micro_recovery_applies_at_inference'],
    'micro_recovery_adds_deployment_parameters': selection['micro_recovery_adds_deployment_parameters'],
    'class_prior_weighting_training_only': True, 'inference_class_prior_changed': False,
...
def export(args):
    ...
    require(not output.exists() and not (run/'PARITY.json').exists(), 'Preserve earlier Micro-recovery export attempts')
    ...
    sources = {str((ROOT/name).resolve()): sha(ROOT/name) for name in
        ('training/export_unified_retention_micro_recovery.py', 'training/train_unified_retention_micro_recovery.py',
         'training/retention_micro_recovery_loss.py', 'training/train_unified_retention_wenkai_recovery.py',
         'training/retention_wenkai_recovery_loss.py', 'training/train_unified_retention_margin.py',
         'training/retention_dual_teacher_loss.py', 'training/retention_confidence_floor_loss.py',
        'training/retention_angular_margin_loss.py', 'training/train_unified_retention_face_balanced.py',
         ...
         'training/evaluate_unified_retention_micro_recovery.py',
         'training/wide_region_network.py',
    ...
    require(step['architecture'] == ARCHITECTURE and step['families'] == selection['families']
            ...
            and state_sha(step['state_dict']) == selection['state_after_sha256'], 'Final angular-margin state differs from the selected saved checkpoint')
    ...
    require(replace_groupnorm(converted, high_precision=True) == 4
            and state_sha(converted.state_dict()) == selection['state_after_sha256'], 'GroupNorm lowering changed angular-margin parameters')
    ...
    for count in (1, 7, 32, 128):
        ...
        require(out.dtype == size.dtype == np.float32 and out.shape == (len(samples), 25) and size.shape == (len(samples),)
                and np.isfinite(out).all() and np.isfinite(size).all(), 'Invalid dynamic angular-margin model output')
    ...
    require(validate(run, data) == selection and all(sha(path) == digest for path, digest in {**sources, **evidence}.items())
            ...
            'Angular-margin evidence or weights changed during export')
