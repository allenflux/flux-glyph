...
def evaluate(args):
    from export_unified_retention_paired_known import validate
    ...
    bindings = {str(path.resolve()): sha(path) for path in [
        ...
        ROOT / 'training/prepare_unified_regions.py', ROOT / 'training/export_unified_retention_paired_known.py',
