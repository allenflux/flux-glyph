#!/usr/bin/env python3
"""Train one image CNN with explicit soft unknown targets and known-only retention KL."""
...
SEED=2026091409
SOFT_UNKNOWN_SUPERVISION={'unknown_target_probability':.5,'named_target_probability':.5/24,
    'named_class_count':24,'applies_to':'TRAIN rows whose true class is __unknown__',
    'known_label_smoothing':.03,'labels_unchanged':True,'inference_rule':False}
OBJECTIVE_VARIANT='full_cnn_soft_unknown_supervision_known_only_core_kl'
OBJECTIVE={'family_cross_entropy_label_smoothing':.03,'native_core_weighting':core.NATIVE_CORE_WEIGHTING,
    ...
    'training_inputs':['image_tiles'],'class_prior_weighting':False,'soft_unknown_supervision':SOFT_UNKNOWN_SUPERVISION}
...
def supervision_targets(targets,families,dtype):
    import torch
    core.registry(families)
    require(len(families)==25 and targets.shape==(96,) and targets.dtype==torch.int64
            and dtype in (torch.float32,torch.float64) and bool(((targets>=0)&(targets<len(families))).all()),
            'Soft supervision requires the complete 25-class 96-row label batch')
    distribution=torch.nn.functional.one_hot(targets,25).to(dtype=dtype)*.97+.03/25
    unknown=families.index(core.UNKNOWN)
    soft=torch.full((25,),.5/24,dtype=dtype,device=targets.device);soft[unknown]=.5
    return torch.where((targets==unknown)[:,None],soft[None,:],distribution)


def full_losses(logits,ratios,targets,sizes,teacher_logits,rows,families):
    import torch
    require(logits.shape==(96,25) and teacher_logits.shape==logits.shape and ratios.shape==targets.shape==sizes.shape==(96,)
            ...
            'Soft-supervised tensors must share shape/device and frozen teacher/native targets')
    require(all(bool(torch.isfinite(value).all()) for value in (logits,ratios,sizes,teacher_logits)),
            'Nonfinite soft-supervised inputs')
    distribution=supervision_targets(targets,families,logits.dtype)
    ...
    ce=(-(distribution*logits.log_softmax(1)).sum(1)*weight).sum()/96
    size_loss=torch.nn.functional.smooth_l1_loss(ratios,sizes)
    mask=(targets!=families.index(core.UNKNOWN)) & (teacher_logits.argmax(1)==targets)
    ...
    require(bool(torch.isfinite(total)),'Nonfinite soft unknown loss')
...
class FullSupplementCounts(core.DistillationCounter):
    ...
    def update(self,rows,mask):
        ...
        require(all(not eligible or row['family']!=core.UNKNOWN for row,eligible in zip(rows,mask)),
                'Unknown targets must never receive cached teacher KL')
        super().update(rows,mask);self.supplement_rows+=len(extra)
        self.supplement_sources.update(r['source_font_family'] for r in extra)
    def report(self):
        return {**super().report(),'schema':'flux-glyph-retention-soft-unknown-counts-v1',
            'objective_variant':OBJECTIVE_VARIANT,'mask_rule':OBJECTIVE['teacher_mask'],'objective':OBJECTIVE,
            'soft_unknown_supervision':SOFT_UNKNOWN_SUPERVISION,
...
def validate_training_counts(report,families,steps):
    require(report.get('schema')=='flux-glyph-retention-soft-unknown-counts-v1'
            and report.get('objective_variant')==OBJECTIVE_VARIANT and report.get('objective')==OBJECTIVE
            and report.get('soft_unknown_supervision')==SOFT_UNKNOWN_SUPERVISION
...
def train(args):
    ...
    paths=[Path(__file__),ROOT/'training/retention_adapter_network.py',ROOT/'training/train_unified_retention_core.py',
        ...
        ROOT/'src/flux_glyph/unified_font.py',ROOT/'src/flux_glyph/region_font.py',ROOT/'training/evaluate_unified_retention_soft_unknown.py',ROOT/'training/retention_supplement_sampler.py',
    ...
    protocol={'schema':'flux-glyph-unified-retention-soft-unknown-protocol-v1','architecture':ARCHITECTURE,'families':families,
        'objective_variant':OBJECTIVE_VARIANT,'objective':OBJECTIVE,'soft_unknown_supervision':SOFT_UNKNOWN_SUPERVISION,
        'sampling':SAMPLING,'fixed_runtime':FIXED_RUNTIME,
    ...
    selection={**protocol,'schema':'flux-glyph-unified-retention-soft-unknown-selection-v1','selected':selected,'history':history,
...
def parser():
    ...
    p.add_argument('--output',type=Path,default=ROOT/'artifacts/unified-font-v2/run-soft-unknown-v1')
