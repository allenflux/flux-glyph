...
PARITY_SCHEMA = 'flux-glyph-unified-native-oe-onnx-parity-v1'
...
def validate_v1_objective(objective):
    from train_unified_native_oe import OBJECTIVE as NATIVE_OBJECTIVE
    require(objective == NATIVE_OBJECTIVE
            ...
            'Native focus supervision or original-unknown teacher policy differs')
...
def validate_counts(counts, families):
    ...
    require(type(counts.get('focus_teacher_eligible_rows')) is int
            and 0 <= counts['focus_teacher_eligible_rows'] <= STEPS * 32,
            'Native focus teacher eligibility exceeds actual input rows')
...
def reconstruct_sampling(datasets, pools, teachers, steps):
    """Reproduce the fixed TRAIN RNG and teacher eligibility without reading pixels."""
    import train_unified_native_oe as trainer
    ...
    return replay.report(), focus.report(), eligible
...
def validate(run, data, plan_path):
    ...
    require(selection.get('schema') == 'flux-glyph-unified-native-oe-selection-v1'
            and protocol.get('schema') == 'flux-glyph-unified-native-oe-training-freeze-v1'
            and plan.get('schema') == 'flux-glyph-unified-native-oe-training-plan-v1'
    ...
    require(bindings.get(str(pre_path)) == pre.get('sha256') == sha(pre_path)
            and proof.get('schema') == 'flux-glyph-native-oe-gradient-preflight-v1'
            ...
            and all(row.get('parts', {}).get('unknown_oe_rows') == 16
                    and row.get('parts', {}).get('unknown_oe', -1) >= 0
    ...
    _, preflight_focus, _ = reconstruct_sampling(datasets, pools, teachers, 4)
    require(proof.get('focus_sampling') == preflight_focus, 'Actual preflight focus RNG or sample accounting differs')
    ...
    replay_report, focus_report, focus_eligible = reconstruct_sampling(datasets, pools, teachers, STEPS)
    require(counts['replay_sampling'] == replay_report and counts['focus_sampling'] == focus_report
            ...
            'Actual fixed TRAIN sampler/teacher trajectory is not reproducible')
...
def training_summary(selection, selection_sha256, checkpoint_sha256):
    ...
    return {'kind': 'R22 continuation with native short-line focus and conditional named-uniformity on replay unknown fonts',
        'steps': STEPS,
        ...
        'derived_crops_used': False, 'cnn_input_rows_per_step': 128,
        'unknown_oe_policy': copy.deepcopy(UNKNOWN_OE_POLICY),
        'unknown_oe_rows': STEPS * 16,
...
def export(args):
    ...
    sources = {str(path.resolve()): sha(path) for path in [Path(__file__),
        ...
        ROOT / 'training/evaluate_unified_native_oe.py', ROOT / 'training/train_unified_native_oe.py',
    ...
    initializer = torch.load(__import__('train_unified_native_oe').BASE_RUN / 'model.pth',
    ...
    validate_initializer_checkpoint(initializer, selection,
        __import__('train_unified_native_oe').BASE_STATE_SHA,
        __import__('train_unified_native_oe').BASE_SELECTION_SHA)
    ...
    retention = read(Path(__import__('train_unified_native_oe').RETENTION_PLAN))
