...
PARITY_SCHEMA = 'flux-glyph-unified-native-short-onnx-parity-v1'
...
def validate_v1_objective(objective):
    from train_unified_native_short import OBJECTIVE as NATIVE_OBJECTIVE
...
def reconstruct_sampling(datasets, pools, teachers, steps):
    ...
    import train_unified_native_short as trainer
...
def validate(run, data, plan_path):
    ...
    require(selection.get('schema') == 'flux-glyph-unified-native-short-selection-v1'
            and protocol.get('schema') == 'flux-glyph-unified-native-short-training-freeze-v1'
            and plan.get('schema') == 'flux-glyph-unified-native-short-training-plan-v1'
    ...
    require(bindings.get(str(pre_path)) == pre.get('sha256') == sha(pre_path)
            and proof.get('schema') == 'flux-glyph-native-short-gradient-preflight-v1'
...
def training_summary(selection, selection_sha256, checkpoint_sha256):
    ...
    return {'kind': 'R22 continuation with complete native short-line focus and same-tile teachers',
        'steps': STEPS,
...
def export(args):
    ...
    sources = {str(path.resolve()): sha(path) for path in [Path(__file__),
        ...
        ROOT / 'training/evaluate_unified_native_short.py', ROOT / 'training/train_unified_native_short.py',
        ROOT / 'training/native_short_focus.py', ROOT / 'training/restore_unknown_train_teacher.py',
    ...
    initializer = torch.load(__import__('train_unified_native_short').BASE_RUN / 'model.pth',
    ...
    validate_initializer_checkpoint(initializer, selection,
        __import__('train_unified_native_short').BASE_STATE_SHA,
        __import__('train_unified_native_short').BASE_SELECTION_SHA)
    ...
    retention = read(Path(__import__('train_unified_native_short').RETENTION_PLAN))
