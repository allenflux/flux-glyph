#!/usr/bin/env python3
"""Export one confidence-floor CNN with exact b08 TRAIN teacher provenance."""
...
def validate_objective_plan(selection, retention_plan_path):
    ...
    return trainer.read_objective_plan(path, sha(retention_plan_path))
...
def validate_fixed_final_selection(selection):
    ...
    require([row['step'] for row in history] == [3000]
            and selection['checkpoint_selection'] ==
                'Fixed final step 3000; no intermediate CAL inference or checkpoint search'
            ...
            'Confidence-floor selection must be the sole fixed final checkpoint')
    return history


def validate_student_initializer(selection):
    """The complete source-balanced student is also the new frozen TRAIN teacher."""
    import train_unified_retention_confidence_floor as trainer
    ...
    require(all(selection[key] == previous[key] for key in ('families', 'base_checkpoint', 'base_selection',
        ...
        and identity['state_sha256'] == previous['state_after_sha256'] == selection['initial_state_sha256'] == selection['state_before_sha256']
        and identity['parameter_groups_sha256'] == previous['selected_parameter_groups_sha256'] == selection['initial_parameter_groups_sha256']
        and previous['initial_state_sha256'] == selection['base_state_sha256']
        and selection['initial_state_sha256'] == selection['cached_teacher_state_sha256'] != selection['base_state_sha256']
        and selection['teacher_identity'] == identity
...
def validate(run, data):
    ...
    require(selection.get('schema') == 'flux-glyph-unified-retention-confidence-floor-selection-v1'
    ...
    require(protocol.get('schema') == 'flux-glyph-unified-retention-confidence-floor-protocol-v1'
    ...
    require(all(key not in document for document in (selection, protocol)
            ...
            'Obsolete unknown supervision metadata cannot describe confidence-floor training')
    expected = {'architecture': ARCHITECTURE, 'objective_variant': trainer.OBJECTIVE_VARIANT,
        ...
        'steps': 3000, 'eval_every': 3000, 'batch_size': 96, 'learning_rate': 2e-5, 'minimum_learning_rate': 2e-6,
        'weight_decay': 1e-4, 'gradient_clip_norm': 5., 'seed': 2026091413,
        'base_selected_step': 1000, 'core_optimizer_steps_executed': 1500,
        'source_optimizer_steps_executed': 3000, 'source_selected_step': 1500, 'optimizer_steps_executed': 3000,
    ...
    require(base.get('schema') == 'flux-glyph-unified-retention-selection-v1'
            ...
            and base['state_after_sha256'] == selection['base_state_sha256'] != selection['cached_teacher_state_sha256']
    ...
    required = [ROOT/name for name in ('training/train_unified_retention_confidence_floor.py',
        'training/retention_confidence_floor_loss.py', 'training/cache_unified_student_teacher.py',
        ...
        'training/evaluate_unified_retention_confidence_floor.py', 'src/flux_glyph/unified_font.py', 'src/flux_glyph/region_font.py')]
...
def validate_known_supplement(selection, original_rows, unknown_rows):
    """Validate historical core-cache provenance and unchanged paired known targets."""
    import train_unified_retention_confidence_floor as trainer
    bindings = selection['bindings']
    data_path = source_entry(selection['known_data'], bindings)
    cache_path = source_entry(selection['known_cache'], bindings)
    require(sha(data_path) == trainer.KNOWN_MANIFEST_SHA and sha(cache_path) == trainer.KNOWN_CACHE_SHA,
            'Paired known data or historical core cache differs from the frozen capture')
    arrays, cache = trainer.load_known_cache(cache_path.parent)
    identity = cache['identity']
    require(identity['data_manifest'] == selection['known_data'] and identity['base_checkpoint'] == selection['base_checkpoint']
            and identity['base_selection'] == selection['base_selection']
            and identity['base_state_sha256'] == selection['base_state_sha256']
            and identity['old_cache_manifest'] == selection['cache_manifest']
            and identity['families'] == selection['families'] and identity['partition']['split'] == 'train'
            and all(bindings.get(path) == digest for path, digest in identity['bindings'].items()),
            'Historical paired known core cache has a different model, registry or source')
    for name in ('data_manifest', 'preparation_freeze', 'partition_manifest', 'old_cache_manifest', 'old_cache_freeze'):
        source_entry(identity[name], bindings)
    source_entry({'path': str(cache_path.parent/'CACHE_FREEZE.json'), 'sha256': cache['cache_freeze_sha256']}, bindings)
    for item in cache['partitions']['train'].values():
    ...
    for key in ('metadata', 'array'):
        source_entry({'path': str((part_path.parent/part[key]['path']).resolve()), 'sha256': part[key]['sha256']}, bindings)
    rows = trainer.read(part_path.parent/part['metadata']['path'])
    require(len(rows) == part['views'] == identity['partition']['region_count']
            and len(arrays['base_logits']) == part['tiles'] == identity['partition']['tile_count']
            and all(row.get('family') in trainer.KNOWN_FAMILIES and row.get('target') == selection['families'].index(row['family'])
                    and row.get('source_font_family') == row['family'] and row.get('split') == 'train'
                    ...
                    and trainer.SUPPLEMENT_MARKER not in row and trainer.KNOWN_SUPPLEMENT_MARKER not in row for row in rows)
            and {row['family'] for row in rows} == set(trainer.KNOWN_FAMILIES), 'Invalid or relabeled paired known TRAIN rows')
    ...
    require(not {(row['source_id'], row['region_id']) for row in rows}.intersection(old | set(paired)),
            'Paired known images overlap an existing TRAIN source')
    for row in rows:
        ...
        require(counterpart is not None and counterpart['family'] == pair['training_family'] == '__unknown__'
                ...
                'Known/unknown pair does not reference the actual separately bound unknown TRAIN region')
    return rows, part


def validate_training_teacher(selection, data):
    """Require freshly inferred b08 logits for all three actual TRAIN tile orders."""
    import train_unified_retention_confidence_floor as trainer
    from cache_unified_student_teacher import load_cache, PARTITIONS, ORDER
    bindings = selection['bindings']; path = source_entry(selection['teacher_cache'], bindings)
    require(isinstance(trainer.TEACHER_CACHE_SHA, str) and sha(path) == trainer.TEACHER_CACHE_SHA
            and selection['teacher_identity'] == selection['student_initializer']
            and selection['cached_teacher_state_sha256'] == selection['teacher_identity']['state_sha256']
            == selection['initial_state_sha256'] != selection['base_state_sha256']
            and selection['historical_core_caches_used_for_training'] is False,
            'Training logits must come from the same b08 initializer, not historical core caches')
    arrays, manifest = load_cache(path.parent)
    identity = manifest['identity']
    require(identity['teacher'] == selection['teacher_identity'] and identity['families'] == selection['families']
    ...
    roots = {'original': Path(data).resolve(),
        ...
        'known': source_entry(selection['known_data'], bindings).parent}
    total = 0
    for name in PARTITIONS:
        ...
        total += part['tiles']
    require(identity['total_tiles'] == total == selection['training_data_counts']['combined_tiles'],
            'Teacher outputs do not cover the full three-source TRAIN union')
...
def validate_counts(run, selection, data):
    ...
    require(all(key not in document for document in (counts, sampling)
                for key in ('soft_unknown_supervision', 'unknown_binary_supervision')),
            'Obsolete unknown supervision metadata in confidence-floor evidence')
    ...
    require(counts.get('teacher_mask_verified_against_same_tile_argmax') is True,
            'TRAIN teacher masks must be checked against the actual same-tile b08 argmax')
    slots = {key: trainer.SAMPLING[key]*3000 for key in ('base_known', 'known_supplement',
    ...
    require(sampling['schema'] == 'flux-glyph-retention-paired-known-sampling-v1'
            and sampling.get('source_balanced') is True
            and sampling['slots'] == slots and sampling['base']['known_rows'] == 138000
            ...
            and counts['supplement_rows'] == new_total and counts['original_rows'] == 282000-new_total
            and counts['supplement_source_rows'] == expected_new
            and sampling.get('known_supplement_rows') == counts['known_supplement_rows'] == 6000
            and sampling.get('known_supplement_source_rows') == counts['known_supplement_source_rows'] == dict.fromkeys(trainer.KNOWN_FAMILIES, 3000)
    ...
    require(dict(views) == sampling['known_supplement_view_rows'] and sum(views.values()) == 6000
...
def training_metadata(selection):
    keys = ('objective_variant', 'objective', 'unknown_floor_supervision', 'sampling', 'unknown_source_order',
        'design_changes', 'single_change_causal_attribution', 'supplement_data', 'supplement_cache', 'known_data', 'known_cache',
        'student_initializer', 'teacher_identity', 'teacher_cache', 'historical_core_caches_used_for_training', 'training_data_counts',
        'objective_plan',
    ...
    return {**{key: copy.deepcopy(selection[key]) for key in keys}, 'selected_step': selection['selected']['step'],
        ...
        'teacher_mask_verified_against_same_tile_argmax': True, 'teacher_distribution_kl': False,
...
def metadata_for_export(selection, cal, parent_metadata, model_sha256, checkpoint_sha256,
    ...
    return {'schema': 'flux-glyph-unified-region-font-v1', 'algorithm': 'unified-region-cnn64x256-v1',
        ...
        **copy.deepcopy(FIXED_RUNTIME), 'font_label_groups': cal['manifest']['font_label_groups'],
        ...
        'validation': {'kind': 'full_cnn_monotone_confidence_floor_calibration_only_at_export',
...
def export(args):
    ...
    sources = {str((ROOT/name).resolve()): sha(ROOT/name) for name in
        ('training/export_unified_retention_confidence_floor.py', 'training/train_unified_retention_confidence_floor.py',
         'training/retention_confidence_floor_loss.py',
         'training/evaluate_unified_retention_confidence_floor.py',
    ...
    require(initial.get('families') == selection['families'] and initial.get('architecture') == ARCHITECTURE
            and initial.get('selection_sha256') == selection['student_initializer']['selection']['sha256']
            and state_sha(initial['state_dict']) == selection['initial_state_sha256'] == selection['student_initializer']['state_sha256']
            == selection['cached_teacher_state_sha256'] != selection['base_state_sha256']
            and selection['teacher_identity'] == selection['student_initializer'],
            'Student and frozen TRAIN teacher are not the same exact prior rank-selected CNN')
    for name in GROUPS:
        group = {key: value for key, value in initial['state_dict'].items() if key.startswith(name+'.')}
    ...
    report = {'schema': SCHEMA, 'passed': True, 'promotion_allowed': True, 'font_model_count': 1, 'encoder_count': 1,
        ...
        'teacher_identity': copy.deepcopy(selection['teacher_identity']),
        'teacher_cache': copy.deepcopy(selection['teacher_cache']),
        ...
        'teacher_logits_used': True, 'teacher_distribution_kl': False,
