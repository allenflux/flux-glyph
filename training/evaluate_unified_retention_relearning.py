#!/usr/bin/env python3
"""Post-freeze development regression for the unified named-teacher relearning trial; never a blind test."""
...
def evaluate(args):
    from export_unified_retention_relearning import validate
    selection = validate(args.run, args.data)
    require(selection.get('promotion_allowed') is True, 'Named-relearning trial has not passed its frozen CAL retention conditions')
    ...
    require(parity.get('schema') == 'flux-glyph-unified-retention-onnx-parity-v1'
            ...
            'Named-relearning export provenance differs')
    ...
    require(model.output_families == selection['families'] and parity['output_family_count'] == len(model.output_families),
            'Named-relearning family order differs')
    ...
    bindings = {str(path.resolve()): sha(path) for path in [
        ...
        ROOT / 'training/prepare_unified_regions.py', ROOT / 'training/export_unified_retention_relearning.py',
