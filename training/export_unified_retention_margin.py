#!/usr/bin/env python3
"""Export one wide CNN trained with a known-font angular-margin auxiliary."""
...
def validate_objective_plan(selection, retention_plan_path):
    ...
    import train_unified_retention_margin as trainer
...
def validate(run, data):
    ...
    require(selection.get('schema') == 'flux-glyph-unified-retention-wide-angular-margin-selection-v1'
            and selection.get('architecture') == ARCHITECTURE and selection.get('promotion_allowed') is True
            and not (run/'NO_PROMOTABLE_CHECKPOINT.json').exists(), 'Angular-margin run has no promotable frozen checkpoint')
    protocol = trainer.read(run/'TRAINING_FREEZE.json')
    require(protocol.get('schema') == 'flux-glyph-unified-retention-wide-angular-margin-protocol-v1'
            ...
            'Angular-margin protocol and selection differ')
    ...
    required = [ROOT/name for name in ('training/train_unified_retention_margin.py',
        ...
        'training/evaluate_unified_retention_margin.py', 'src/flux_glyph/unified_font.py', 'src/flux_glyph/region_font.py')]
...
def training_metadata(selection):
    ...
    keys = ('angular_margin', 'angular_margin_applies_at_inference',
        'angular_margin_adds_deployment_parameters', 'historical_known_data', 'historical_known_cache', 'known_cache_kind',
...
def metadata_for_export(selection, cal, parent_metadata, model_sha256, checkpoint_sha256,
    ...
    return {'schema': 'flux-glyph-unified-region-font-v1', 'algorithm': 'unified-region-cnn64x256-v1',
        ...
        **copy.deepcopy(FIXED_RUNTIME), 'font_label_groups': cal['manifest']['font_label_groups'],
        ...
        'validation': {'kind': 'full_cnn_training_angular_margin_calibration_only_at_export',
...
def export(args):
    ...
    require(not output.exists() and not (run/'PARITY.json').exists(), 'Preserve earlier angular-margin export attempts')
    ...
    sources = {str((ROOT/name).resolve()): sha(ROOT/name) for name in
        ('training/export_unified_retention_margin.py', 'training/train_unified_retention_margin.py',
         'training/retention_dual_teacher_loss.py', 'training/retention_confidence_floor_loss.py',
        'training/retention_angular_margin_loss.py', 'training/train_unified_retention_face_balanced.py',
         'training/train_unified_retention_confidence_floor.py',
         'training/evaluate_unified_retention_margin.py', 'training/wide_region_network.py',
