#!/usr/bin/env python3
"""Train one native CNN with unchanged mobile CE plus matched-font pair margins."""
...
def context(args):
    ...
    return (model, datasets, replay, sampler, focus_sampler, mobile, mobile_sampler, base_selection,
            ...
            pair_sampler, pair_proof, pair_reconstruction)
...
def design(args, bindings):
    return {'schema': 'flux-glyph-unified-native-pairs-training-plan-v1', 'architecture': ARCHITECTURE,
...
def preflight(args):
    ...
    (model, datasets, replay, sampler, focus_sampler, mobile, mobile_sampler, _, _, focus_proof,
     mobile_proof, reconstruction, pair_sampler, pair_proof, pair_reconstruction) = context(args)
    ...
    report = {'schema': 'flux-glyph-native-pairs-gradient-preflight-v1', 'passed': True, 'steps': 4,
              'device': args.device, 'replay_tiles': 384, 'focus_tiles': 128, 'mobile_tiles': 80,
...
def train(args):
    ...
    (model, datasets, replay, sampler, focus_sampler, mobile, mobile_sampler, base_selection,
     teacher_proof, focus_proof, mobile_proof, reconstruction,
     pair_sampler, pair_proof, pair_reconstruction) = context(args)
    bindings = files_bound(args, datasets)
    ...
    protocol = {**planned, 'schema': 'flux-glyph-unified-native-pairs-training-freeze-v1', 'bindings': bindings,
    ...
    selection = {**protocol, 'schema': 'flux-glyph-unified-native-pairs-selection-v1', 'selected': record,
