#!/usr/bin/env python3
"""Train one image CNN with balanced unknown sources and binary unknown supervision."""
...
from retention_source_balanced_sampler import SourceBalancedSampler,SAMPLING
...
SEED=2026091410
UNKNOWN_BINARY_SUPERVISION={'unknown_probability_target':.5,'log_odds':'z_unknown - logsumexp(z_named)',
    'applies_to':'TRAIN rows whose true class is __unknown__','named_conditional_distribution_target':None,
    'known_label_smoothing':.03,'labels_unchanged':True,'inference_rule':False}
OBJECTIVE_VARIANT='full_cnn_source_balanced_unknown_binary_supervision'
DESIGN_CHANGES=['Balance all 11 true unknown source families across 16 slots.',
    'Replace the unknown soft-uniform class target with binary known/unknown probability supervision.']
OBJECTIVE={'family_cross_entropy_label_smoothing':.03,'native_core_weighting':core.NATIVE_CORE_WEIGHTING,
    'unknown_loss_weight':1.,'ce_denominator':96,'size_smooth_l1_weight':.2,'size_loss_weighted':False,
    'teacher_kl_weight':2.,'teacher_temperature':1.,'teacher_logits_used':True,'second_model_resident':False,
    'teacher_mask':'TRAIN true target is known and outside PingFang/SF Pro/Helvetica, and same-tile frozen core argmax equals truth',
    ...
    'training_inputs':['image_tiles'],'class_prior_weighting':False,'unknown_binary_supervision':UNKNOWN_BINARY_SUPERVISION}
...
def unknown_binary_loss(logits,families):
    import torch
    core.registry(families)
    require(len(families)==25 and logits.ndim==2 and logits.shape[1]==25 and len(logits)>0
            and logits.dtype in (torch.float32,torch.float64) and bool(torch.isfinite(logits).all()),
            'Unknown binary supervision requires finite logits over all 25 classes')
    unknown=families.index(core.UNKNOWN)
    named=torch.cat((logits[:,:unknown],logits[:,unknown+1:]),dim=1)
    log_odds=logits[:,unknown]-torch.logsumexp(named,dim=1)
    return torch.nn.functional.binary_cross_entropy_with_logits(log_odds,torch.full_like(log_odds,.5),reduction='none')
...
def full_losses(logits,ratios,targets,sizes,teacher_logits,rows,families):
    ...
    if bool(unknown.any()):per_row[unknown]=unknown_binary_loss(logits[unknown],families)
    ce=(per_row*weight).sum()/96
    size_loss=torch.nn.functional.smooth_l1_loss(ratios,sizes)
    mask=known & (teacher_logits.argmax(1)==targets)
    for family in core.CORE_FAMILIES:mask&=targets!=families.index(family)
    if bool(mask.any()):
...
class FullSupplementCounts(core.DistillationCounter):
    ...
    def update(self,rows,mask):
        ...
        for row in rows:
            marker=row.get(SUPPLEMENT_MARKER,False)
            require(type(marker) is bool and marker==(row['family']==core.UNKNOWN and row['source_font_family'] in NEW_SOURCES),
    ...
    def report(self):
        return {**super().report(),'schema':'flux-glyph-retention-source-balanced-counts-v1',
...
def validate_training_counts(report,families,steps):
    ...
    require(report.get('schema')=='flux-glyph-retention-source-balanced-counts-v1'
            ...
            and report.get('original_rows')==steps*96-sum(extra.values())
...
def train(args):
    ...
    paths=[Path(__file__),ROOT/'training/retention_adapter_network.py',ROOT/'training/train_unified_retention_core.py',
        ...
        ROOT/'src/flux_glyph/unified_font.py',ROOT/'src/flux_glyph/region_font.py',ROOT/'training/evaluate_unified_retention_source_balanced.py',ROOT/'training/retention_supplement_sampler.py',ROOT/'training/retention_source_balanced_sampler.py',
    ...
    extra_paths=[Path(__file__),ROOT/'training/retention_supplement_sampler.py',ROOT/'training/retention_source_balanced_sampler.py',
    ...
    sampler=SourceBalancedSampler(training['rows'],families,args.seed,plan['train_pools'],supplement['rows'])
    protocol={'schema':'flux-glyph-unified-retention-source-balanced-protocol-v1','architecture':ARCHITECTURE,'families':families,
    ...
    selection={**protocol,'schema':'flux-glyph-unified-retention-source-balanced-selection-v1','selected':selected,'history':history,
...
def parser():
    ...
    p.add_argument('--output',type=Path,default=ROOT/'artifacts/unified-font-v2/run-source-balanced-v1')
