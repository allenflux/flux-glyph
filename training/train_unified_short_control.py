#!/usr/bin/env python3
"""Bounded replay-only control for R22 short-region continuation."""
...
OBJECTIVE = {
    ...
    'short_loss': None, 'short_teacher': None, 'short_weight': 0.,
    'short_size_beta': None, 'replay_size_beta': 1.,
    'short_forwarded_rows': 32, 'short_supervised_rows': 0,
    'replay_supervision_rows_per_step': 96,
    'control_purpose': 'replay-only attribution control; short rows forwarded for shape/RNG comparability',
    ...
    'additional_confidence_retention': None,
    'original_full_correct_teacher_kl_preserved': True,
    'full_losses_replay_only': True,
}
...
def files_bound(args, datasets):
    ...
    files = [Path(__file__), ROOT / 'training/train_unified_short_regions.py',
             ROOT / 'artifacts/unified-font-v4/short-v2-next-audit/REPORT.json',
...
def objective(model, batch):
    ...
    # The short rows are deliberately forwarded, but contribute exactly zero.
    short_zero = font[96:].sum() * 0.0 + size[96:].sum() * 0.0 + features[96:].sum() * 0.0
    return replay[0] + short_zero, {'replay': replay[0], 'replay_ce': replay[1],
                                   'teacher_kl': replay[3], 'short_forward_zero': short_zero,
                                   'short_supervised': short_zero}


def design(args, bindings):
    return {'schema': 'flux-glyph-unified-short-control-plan-v1', 'architecture': ARCHITECTURE,
...
def preflight(args):
    ...
    report = {'schema': 'flux-glyph-short-control-gradient-preflight-v1', 'passed': True, 'steps': 4,
              'device': args.device, 'replay_tiles': 384, 'short_forwarded_tiles': 128,
              'short_supervised_tiles': 0, 'traces': traces,
...
def train(args):
    ...
    protocol = {**planned, 'schema': 'flux-glyph-unified-short-control-freeze-v1', 'bindings': bindings,
    ...
    training_counts = {'optimizer_steps': STEPS, 'replay_rows': sum(counts.values()),
                       'short_rows': sum(short_counts.values()), 'short_supervised_rows': 0,
                       'replay_supervision_rows': sum(counts.values()), 'replay_by_family': dict(counts),
    ...
    selection = {**protocol, 'schema': 'flux-glyph-unified-short-control-selection-v1', 'selected': record,
