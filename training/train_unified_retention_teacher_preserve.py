#!/usr/bin/env python3
"""Train one image CNN while preserving all truth-correct b08 TRAIN predictions."""
...
SEED=2026091412
UNKNOWN_BINARY_SUPERVISION=previous.UNKNOWN_BINARY_SUPERVISION
OBJECTIVE_VARIANT='full_cnn_paired_known_truth_correct_teacher_preservation'
DESIGN_CHANGES=['Replace the historical core teacher with the same b08 checkpoint used to initialize the student.',
    'Preserve every truth-correct TRAIN teacher prediction, including core known classes and unknown.']
OBJECTIVE={**previous.OBJECTIVE,
    'teacher_mask':'TRAIN same-tile frozen b08 argmax equals true target, including all known classes and unknown',
    'teacher_kl_reduction':'Mean eligible KL(b08 teacher || student); differentiable zero if empty'}
...
def full_losses(logits,ratios,targets,sizes,teacher_logits,rows,families):
    ...
    require(len(families)==25 and logits.shape==(96,25) and teacher_logits.shape==logits.shape
            ...
            'Binary unknown tensors must share shape/device and frozen teacher/native targets')
    require(all(bool(torch.isfinite(value).all()) for value in (logits,ratios,sizes,teacher_logits)),
            'Nonfinite binary unknown inputs')
    ...
    if bool(unknown.any()):per_row[unknown]=previous.unknown_binary_loss(logits[unknown],families)
    ce=(per_row*weight).sum()/96
    size_loss=torch.nn.functional.smooth_l1_loss(ratios,sizes)
    mask=teacher_logits.argmax(1)==targets
    if bool(mask.any()):
        kl=torch.nn.functional.kl_div(logits[mask].log_softmax(1),teacher_logits[mask].softmax(1),reduction='none').sum(1).mean()
    else:kl=logits.sum()*0.
    total=ce+.2*size_loss+2*kl
    require(bool(torch.isfinite(total)),'Nonfinite source-balanced loss')
...
class FullSupplementCounts(TruthCorrectCounter):
    ...
    def report(self):
        return {**super().report(),'schema':'flux-glyph-retention-teacher-preserve-counts-v1',
            'objective_variant':OBJECTIVE_VARIANT,'mask_rule':OBJECTIVE['teacher_mask'],'objective':OBJECTIVE,
            'unknown_binary_supervision':UNKNOWN_BINARY_SUPERVISION,
...
def validate_training_counts(report,families,steps):
    ...
    require(report.get('schema')=='flux-glyph-retention-teacher-preserve-counts-v1'
            and report.get('objective_variant')==OBJECTIVE_VARIANT and report.get('objective')==OBJECTIVE
            and report.get('unknown_binary_supervision')==UNKNOWN_BINARY_SUPERVISION
...
def load_student_initializer(path,model,families):
    """Whole prior rank-selected b08 student; the new TRAIN teacher uses this same checkpoint."""
    import torch
    path=Path(path).resolve();folder=path.parent
    require(sha(path)==STUDENT_CHECKPOINT_SHA and sha(folder/'SELECTION.json')==STUDENT_SELECTION_SHA,
        'Use the exact source-balanced selected student, not an averaged or last-step checkpoint')
    selected=read(folder/'SELECTION.json');protocol=read(folder/'TRAINING_FREEZE.json');report=read(folder/'report.json')
    require(selected['schema']=='flux-glyph-unified-retention-source-balanced-selection-v1'
        and selected['objective_variant']==previous.OBJECTIVE_VARIANT and selected['objective']==previous.OBJECTIVE
        and sha(folder/'TRAINING_FREEZE.json')==selected['training_protocol_sha256']
        and all(selected.get(k)==v for k,v in protocol.items() if k!='schema')
        and selected['selected']==max(selected['history'],key=retention_rank)
        and selected['selected']['step']==1500 and selected['optimizer_steps_executed']==3000
        and [r['step'] for r in selected['history']]==list(range(500,3001,500))
        and selected['promotion_allowed'] is False and selected['families']==families
        and selected['fixed_runtime']==FIXED_RUNTIME and report['status']=='NO_PROMOTABLE_CHECKPOINT'
        and report['optimizer_steps_executed']==3000 and report['selected_step']==1500
        and report['selected_checkpoint_sha256']==STUDENT_CHECKPOINT_SHA
        and all(doc.get(k) is False for doc in (selected,protocol,report) for k in ('test_read','development_holdout_read')),
        'Prior student completion, rank, fixed policy, or source scope changed')
    bindings=dict(selected['bindings']);core.verify_bindings(bindings)
    for record in selected['history']:
        for item in record['artifacts'].values():
            p=(folder/item['path']).resolve();require(p.is_relative_to(folder) and sha(p)==item['sha256'],'Initializer step evidence changed')
    ...
    require(checkpoint.get('selection_sha256')==STUDENT_SELECTION_SHA
        and state_sha(checkpoint['state_dict'])==selected['state_after_sha256']
        and parameter_groups(checkpoint['state_dict'])==selected['selected_parameter_groups_sha256'],
        'Prior student state/group identity changed')
    inheritance=initialize(model,checkpoint,families)
    identity={'checkpoint':{'path':str(path),'sha256':STUDENT_CHECKPOINT_SHA},
        'selection':{'path':str(folder/'SELECTION.json'),'sha256':STUDENT_SELECTION_SHA},
        'training_freeze':{'path':str(folder/'TRAINING_FREEZE.json'),'sha256':selected['training_protocol_sha256']},
        'state_sha256':selected['state_after_sha256'],'parameter_groups_sha256':selected['selected_parameter_groups_sha256'],
        'selected_step':1500,'optimizer_steps_executed':3000}
    return identity,inheritance,bindings,selected


def validate_teacher_data(identity,datasets,teacher):
    """Bind each same-tile target to the actual loaded TRAIN array and row order."""
    from cache_unified_student_teacher import PARTITIONS,ORDER
    require(set(datasets)==set(PARTITIONS)==set(identity.get('partitions',{}))
        and identity.get('teacher')==teacher and teacher['checkpoint']['sha256']==STUDENT_CHECKPOINT_SHA
        and teacher['selection']['sha256']==STUDENT_SELECTION_SHA
        and identity.get('architecture')==ARCHITECTURE and identity.get('families')==datasets['original']['families']
        and identity.get('teacher_logits_recomputed') is True and identity.get('historical_logits_reused') is False
        and identity.get('optimizer_steps_executed')==identity.get('teacher_optimizer_steps_executed')==0
        and all(identity.get(k) is False for k in ('test_read','development_holdout_read','calibration_images_read')),
        'Actual teacher must be b08 over exactly the three TRAIN datasets')
    for name in PARTITIONS:
        data=datasets[name];part=data['partition'];item=identity['partitions'][name]
        require(item.get('split')==part.get('split')=='train'
            and data['families']==identity['families']
            and item['data_manifest']['sha256']==data['manifest_sha256']
            and item['partition_manifest']['sha256']==data['partition_sha256']
            and item['rows']['sha256']==part['metadata']['sha256']
            and item['tiles']['sha256']==part['array']['sha256']
            and item['region_count']==len(data['rows']) and item['tile_count']==len(data['tiles'])
            and item['shape']==list(data['tiles'].shape) and item['order']==ORDER
            and isinstance(data['tiles'],np.memmap)
            and Path(data['tiles'].filename).resolve()==Path(item['tiles']['path']).resolve()
            and data['rows']==read(item['rows']['path']),
            'Teacher cache dataset, pixel file, row order or TRAIN partition differs')
    require(identity.get('total_tiles')==sum(len(data['tiles']) for data in datasets.values()),
        'Teacher tile count differs from the actual TRAIN union')


def load_training_teacher(folder,datasets,teacher,bindings):
    from cache_unified_student_teacher import load_cache as load_teacher_cache
    folder=Path(folder).resolve();manifest_path=folder/'CACHE_MANIFEST.json'
    require(isinstance(TEACHER_CACHE_SHA,str) and len(TEACHER_CACHE_SHA)==64
        and sha(manifest_path)==TEACHER_CACHE_SHA,'Use the immutable recomputed b08 teacher cache')
    arrays,manifest=load_teacher_cache(folder)
    validate_teacher_data(manifest['identity'],datasets,teacher)
    incoming=dict(manifest['identity']['bindings'])
    incoming[str(manifest_path)]=TEACHER_CACHE_SHA
    incoming[str(folder/'CACHE_FREEZE.json')]=sha(folder/'CACHE_FREEZE.json')
    for entries in manifest['partitions'].values():
        for item in entries.values():incoming[str((folder/item['path']).resolve())]=item['sha256']
    for path,digest in incoming.items():
        require(path not in bindings or bindings[path]==digest,'New teacher cache conflicts with historical source closure')
        require(sha(path)==digest,'New teacher cache or source changed');bindings[path]=digest
    return arrays,manifest,{'path':str(manifest_path),'sha256':TEACHER_CACHE_SHA}
...
def train(args):
    ...
    paths=[Path(__file__),ROOT/'training/retention_adapter_network.py',ROOT/'training/train_unified_retention_core.py',
        ...
        ROOT/'src/flux_glyph/unified_font.py',ROOT/'src/flux_glyph/region_font.py',ROOT/'training/evaluate_unified_retention_teacher_preserve.py',ROOT/'training/retention_supplement_sampler.py',ROOT/'training/retention_source_balanced_sampler.py',ROOT/'training/retention_paired_known_sampler.py',ROOT/'training/train_unified_retention_source_balanced.py',
        ...
        ROOT/'training/cache_unified_student_teacher.py',
    ...
    require(sha(known_manifest_path)==KNOWN_MANIFEST_SHA and sha(known_cache_path)==KNOWN_CACHE_SHA,
        'Use only the frozen paired known capture and core teacher cache')
    ...
    for path,digest in student_bindings.items():
        require(path not in bindings or bindings[path]==digest,'Prior student and core teacher source closures conflict')
        bindings[path]=digest
    full_before=state_sha(model.state_dict());initial_groups=parameter_groups(model.state_dict())
    require(full_before==student_identity['state_sha256']!=base_before
        and initial_groups==student_identity['parameter_groups_sha256'],'Student initializer and historical core identities were conflated')
    teacher_cache,teacher_cache_manifest,teacher_cache_identity=load_training_teacher(
        args.teacher_cache,{'original':training,'supplement':supplement,'known':known},student_identity,bindings)
    ...
    protocol={'schema':'flux-glyph-unified-retention-teacher-preserve-protocol-v1','architecture':ARCHITECTURE,'families':families,
        'objective_variant':OBJECTIVE_VARIANT,'objective':OBJECTIVE,'unknown_binary_supervision':UNKNOWN_BINARY_SUPERVISION,
        ...
        'cached_teacher_state_sha256':student_identity['state_sha256'],
        'teacher_identity':student_identity,'teacher_cache':teacher_cache_identity,
    ...
    selection={**protocol,'schema':'flux-glyph-unified-retention-teacher-preserve-selection-v1','selected':selected,'history':history,
...
def parser():
    ...
    p.add_argument('--output',type=Path,default=ROOT/'artifacts/unified-font-v2/run-teacher-preserve-v1')
