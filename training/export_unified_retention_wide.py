#!/usr/bin/env python3
"""Export one wide CNN with deterministic widening and dual TRAIN teachers."""
...
def validate(run, data):
    ...
    require(selection.get('schema') == 'flux-glyph-unified-retention-wide-transfer-selection-v1'
    ...
    require(protocol.get('schema') == 'flux-glyph-unified-retention-wide-transfer-protocol-v1'
    ...
    required = [ROOT/name for name in ('training/train_unified_retention_wide.py',
        ...
        'training/evaluate_unified_retention_wide.py', 'src/flux_glyph/unified_font.py', 'src/flux_glyph/region_font.py')]
...
def validate_training_teacher(selection, data):
    ...
    require(r21_identity == unknown and all(bindings.get(name) == digest for name, digest in replay_bindings.items())
            and replay_mix == mix
            and all(r21_arrays[name].shape == arrays[name]['base_logits'].shape for name in PARTITIONS),
            'R21 unknown logits do not cover the same three TRAIN tile orders as b08')
    return manifest
...
def validate_counts(run, selection, data):
    ...
    families = Counter({family: 12000 for family in selection['families'] if family != '__unknown__'})
    families.update({'__unknown__': 96000, 'PingFang': 96000, 'SF Pro': 24000, 'Helvetica': 24000})
    families.update({family: 12000 for family in trainer.SAMPLING['original_eight_families']})
...
def training_metadata(selection):
    keys = ('objective_variant', 'objective', 'unknown_floor_supervision', 'sampling', 'unknown_source_order',
        ...
        'unknown_teacher_identity', 'teacher_policy', 'teacher_mix', 'widening', 'widening_parity',
...
def metadata_for_export(selection, cal, parent_metadata, model_sha256, checkpoint_sha256,
    ...
    return {'schema': 'flux-glyph-unified-region-font-v1', 'algorithm': 'unified-region-cnn64x256-v1',
        ...
        **copy.deepcopy(FIXED_RUNTIME), 'font_label_groups': cal['manifest']['font_label_groups'],
        ...
        'validation': {'kind': 'full_cnn_monotone_wide_transfer_calibration_only_at_export',
...
def export(args):
    ...
    sources = {str((ROOT/name).resolve()): sha(ROOT/name) for name in
        ('training/export_unified_retention_wide.py', 'training/train_unified_retention_wide.py',
         'training/retention_dual_teacher_loss.py', 'training/retention_confidence_floor_loss.py',
         'training/train_unified_retention_confidence_floor.py',
         'training/evaluate_unified_retention_wide.py', 'training/wide_region_network.py',
