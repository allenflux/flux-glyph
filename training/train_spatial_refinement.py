...
CONFIGS = {'refinement-low': 1.2e-4, 'refinement-medium': 1.6e-4, 'refinement-high': 2.4e-4}
REPAIR_WEIGHTS = {'refinement-low': 1., 'refinement-medium': 1., 'refinement-high': 1.}
STEPS = 2400
...
OBJECTIVE = {**parent.OBJECTIVE, 'all_parameters_trained': False,
             ...
             'rationale': 'Retain balanced supervision with confusion weight 1.0 and learn finer spatial evidence at three higher fixed learning rates.'}
...
def bound_sources(args, datasets, feature_manifest):
    ...
    for path in [Path(__file__), ROOT / 'training/spatial_rejection_network.py',
                 ...
                 ROOT / 'training/train_spatial_balanced.py',
...
def preflight(args):
    ...
    for trial, rate in CONFIGS.items():
        ...
        planned = {'schema': 'flux-glyph-spatial-refinement-plan-v1', 'trial': trial,
    ...
    write_new(args.output / 'GRID.json', {'schema': 'flux-glyph-spatial-refinement-grid-v1',
...
def calibrate(model, run, protocol, counts, residual_report, state):
    ...
    selection = {**protocol, 'schema': 'flux-glyph-spatial-refinement-selection-v1',
...
def train(args):
    ...
    for trial, rate in CONFIGS.items():
        ...
        protocol = {**plan, 'schema': 'flux-glyph-spatial-refinement-training-freeze-v1',
        ...
        if allowed:
            selected = trial
            break
    write_new(args.output / 'GRID_RESULT.json', {'schema': 'flux-glyph-spatial-refinement-grid-result-v1',
