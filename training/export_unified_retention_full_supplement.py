#!/usr/bin/env python3
"""Export a fully trained font-and-size CNN with TRAIN-only cached-teacher provenance."""
...
def validate(run, data):
    ...
    import train_unified_retention_full_supplement as trainer
    run, data = Path(run).resolve(), Path(data).resolve()
    selection = trainer.read(run/'SELECTION.json')
    require(selection.get('schema') == 'flux-glyph-unified-retention-full-supplement-selection-v1'
    ...
    require(protocol.get('schema') == 'flux-glyph-unified-retention-full-supplement-protocol-v1'
    ...
    expected = {'architecture': ARCHITECTURE, 'objective_variant': trainer.OBJECTIVE_VARIANT,
        ...
        'weight_decay': 1e-4, 'gradient_clip_norm': 5., 'seed': 2026091408,
    ...
    required = [ROOT/name for name in ('training/train_unified_retention_full_supplement.py', 'training/retention_supplement_sampler.py',
        ...
        'training/evaluate_unified_retention_full_supplement.py', 'src/flux_glyph/unified_font.py', 'src/flux_glyph/region_font.py')]
...
def training_metadata(selection):
    keys = ('objective_variant', 'objective', 'supplement_data', 'supplement_cache', 'training_data_counts',
...
def metadata_for_export(selection, cal, parent_metadata, model_sha256, checkpoint_sha256,
    ...
    return {'schema': 'flux-glyph-unified-region-font-v1', 'algorithm': 'unified-region-cnn64x256-v1',
        ...
        **copy.deepcopy(FIXED_RUNTIME), 'font_label_groups': cal['manifest']['font_label_groups'],
        ...
        'validation': {'kind': 'full_cnn_native_unknown_supplement_calibration_only_at_export',
...
def export(args):
    ...
    sources = {str((ROOT/name).resolve()): sha(ROOT/name) for name in
        ('training/export_unified_retention_full_supplement.py', 'training/train_unified_retention_full_supplement.py',
