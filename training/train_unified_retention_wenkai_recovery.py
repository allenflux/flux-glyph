#!/usr/bin/env python3
"""Train the candidate18 CNN with extra true-WenKai TRAIN cross entropy."""
...
OBJECTIVE_VARIANT='wide_cnn_face_balanced_angular_margin_wenkai_ce_recovery'
DESIGN_CHANGES=[
    'Add exactly one extra unit of label-smoothed family CE for true LXGW WenKai TRAIN rows, raising their family CE weight from 1 to 2.',
    'Keep candidate 18 full eligible-teacher KL, angular auxiliary, native-core weights, unknown floor, size loss, b08-to-wide initializer, face-balanced replay, data, schedule, architecture and runtime fixed.']
OBJECTIVE={**margin_trial.OBJECTIVE,'class_prior_weighting':True,
...
def read_objective_plan(path, acceptance_plan_sha256, weight_teacher_cache_sha256):
    ...
    expected={'schema':'flux-glyph-wide-wenkai-recovery-plan-v1',
...
def full_losses(logits,ratios,features,family_head_weight,targets,sizes,teacher_logits,rows,families):
    import torch
    base_total,base_ce,size_loss,preservation,auxiliary,mask=margin_trial.full_losses(
        logits,ratios,features,family_head_weight,targets,sizes,teacher_logits,rows,families)
    recovery=wenkai_recovery_extra_ce(logits,targets,rows,families)
...
class FullSupplementCounts(TruthCorrectCounter):
    ...
    def report(self):
        return {**super().report(),'schema':'flux-glyph-retention-wenkai-recovery-counts-v1',
...
def validate_training_counts(report,families,steps):
    ...
    require(report.get('schema')=='flux-glyph-retention-wenkai-recovery-counts-v1'
...
def train(args):
    ...
    paths=[Path(__file__),ROOT/'training/retention_adapter_network.py',ROOT/'training/train_unified_retention_core.py',
        ...
        ROOT/'training/train_unified_retention_margin.py',ROOT/'training/retention_wenkai_recovery_loss.py',
    ...
    protocol={'schema':'flux-glyph-unified-retention-wide-wenkai-recovery-protocol-v1','architecture':ARCHITECTURE,'families':families,
    ...
    for step in range(1,STEPS+1):
        ...
        recovery=wenkai_recovery_extra_ce(logits,targets,rows,families).detach()
    ...
    selection={**protocol,'schema':'flux-glyph-unified-retention-wide-wenkai-recovery-selection-v1','selected':selected,'history':history,
...
def parser():
    ...
    p.add_argument('--objective-plan',type=Path,default=ROOT/'artifacts/unified-font-v3/wenkai-recovery-plan-v1/PLAN.json')
    p.add_argument('--checkpoint',type=Path,default=ROOT/'artifacts/unified-font-v2/run-core-v1/model.pth')
    p.add_argument('--output',type=Path,default=ROOT/'artifacts/unified-font-v3/run-wide-wenkai-recovery-v1')
