...
def evaluate(args):
    from export_unified_retention_source_average import validate
    ...
    bindings = {str(path.resolve()): sha(path) for path in [
        ...
        ROOT / 'training/prepare_unified_regions.py', ROOT / 'training/export_unified_retention_source_average.py',
