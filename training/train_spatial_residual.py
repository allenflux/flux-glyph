#!/usr/bin/env python3
"""Fit zero-sum fine spatial features while preserving every inherited R22 value."""
...
from spatial_residual_network import (POLICY, add_residual, fold_residual, frozen_state,
...
CONFIGS = {'residual-low': 5e-6, 'residual-medium': 2e-5, 'residual-high': 8e-5}
STEPS = 2400
...
OBJECTIVE = {**parent.OBJECTIVE, 'all_parameters_trained': False,
             ...
             'rationale': 'Learn fine horizontal differences with inherited R22 coarse parameters frozen.'}


def bound_sources(args, datasets, feature_manifest):
    bindings = parent.files_bound(args, datasets)
    for path in [Path(__file__), ROOT / 'training/spatial_residual_network.py',
...
def optimizer_step(model, head, batch, optimizer):
    import torch
    loss, parts = parent.objective(head, batch)
    ...
    require(len(trainable) == 1, 'Only one zero-sum residual tensor may train')
...
def preflight(args):
    ...
    for trial, rate in CONFIGS.items():
        ...
        optimizer = torch.optim.AdamW([model.style[0].residual_raw], lr=rate, weight_decay=1e-4)
        ...
        for step in range(4):
            ...
            loss, parts, gradient = optimizer_step(model, head, batch, optimizer)
        ...
        planned = {'schema': 'flux-glyph-spatial-residual-plan-v1', 'trial': trial,
            ...
            'selection_policy': 'fixed final step only; first fully CAL-qualified trial in declared ascending learning-rate order',
            'grid': CONFIGS, 'bindings': bindings, 'test_read': False,
    ...
    write_new(args.output / 'GRID.json', {'schema': 'flux-glyph-spatial-residual-grid-v1',
...
def calibrate(model, run, protocol, counts, residual_report, state):
    ...
    selection = {**protocol, 'schema': 'flux-glyph-spatial-residual-selection-v1',
...
def train(args):
    ...
    for trial, rate in CONFIGS.items():
        ...
        protocol = {**plan, 'schema': 'flux-glyph-spatial-residual-training-freeze-v1',
        ...
        if allowed:
            selected = trial
            break
    write_new(args.output / 'GRID_RESULT.json', {'schema': 'flux-glyph-spatial-residual-grid-result-v1',
