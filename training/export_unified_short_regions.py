...
def validate(run, data, plan_path):
    ...
    import train_unified_short_regions as trainer
    ...
    plan = read(plan_path)
    require(selection.get('schema') == 'flux-glyph-unified-short-selection-v1'
            and protocol.get('schema') == 'flux-glyph-unified-short-training-freeze-v1'
            and plan.get('schema') == 'flux-glyph-unified-short-training-plan-v1'
    ...
    require(bindings.get(str(pre_path)) == pre.get('sha256') == sha(pre_path)
            and proof.get('schema') == 'flux-glyph-short-region-gradient-preflight-v1'
...
def export(args):
    ...
    sources = {str(path.resolve()): sha(path) for path in [Path(__file__),
        ROOT / 'training/evaluate_unified_short_regions.py', ROOT / 'training/train_unified_short_regions.py',
    ...
    initializer = torch.load(__import__('train_unified_short_regions').BASE_RUN / 'model.pth',
    ...
    validate_initializer_checkpoint(initializer, selection,
        __import__('train_unified_short_regions').BASE_STATE_SHA,
        __import__('train_unified_short_regions').BASE_SELECTION_SHA)
    ...
    retention = read(Path(__import__('train_unified_short_regions').RETENTION_PLAN))
