#!/usr/bin/env python3
"""Continue R22 short-region training with correct-teacher confidence retention."""
...
from short_confidence_retention import CONTRACT as CONFIDENCE_RETENTION, confidence_retention_loss
...
OBJECTIVE = {
    ...
    'additional_confidence_retention': CONFIDENCE_RETENTION,
...
def files_bound(args, datasets):
    ...
    files = [Path(__file__), ROOT / 'training/short_confidence_retention.py',
             ROOT / 'training/train_unified_short_regions.py',
             ROOT / 'artifacts/unified-font-v4/short-train-probe-v1/REPORT.json',
             ROOT / 'artifacts/unified-font-v4/short-failure-diagnosis-v2/AUDIT.json',
...
def objective(model, batch):
    ...
    retained, eligible, boosted = confidence_retention_loss(font[:96], batch['teacher'], batch['targets'], batch['rows'])
    return replay[0] + augment[0] + retained, {'confidence_retention': retained,
                                   'confidence_eligible': eligible.sum().float(),
                                   'confidence_boosted': boosted.sum().float(), 'replay': replay[0], 'replay_ce': replay[1], 'teacher_kl': replay[3],
                                   'short_weighted': augment[0], 'short_font': augment[1], 'short_size': augment[2]}


def design(args, bindings):
    return {'schema': 'flux-glyph-unified-short-training-plan-v2', 'architecture': ARCHITECTURE,
...
def preflight(args):
    ...
    report = {'schema': 'flux-glyph-short-region-gradient-preflight-v2', 'passed': True, 'steps': 4,
...
def train(args):
    ...
    protocol = {**planned, 'schema': 'flux-glyph-unified-short-training-freeze-v2', 'bindings': bindings,
    ...
    selection = {**protocol, 'schema': 'flux-glyph-unified-short-selection-v2', 'selected': record,
