#!/usr/bin/env python3
"""Train native short lines with conditional known-uniformity on replay unknown fonts."""
...
OBJECTIVE = {
    ...
    'rationale': 'Native-short-v1 improves seen-unknown TRAIN but worsens unseen-font CAL confidence. Add conditional named-uniformity only on true-unknown replay rows; retain original native focus and full losses.',
...
def files_bound(args, datasets):
    ...
    files = [Path(__file__), ROOT / 'training/native_short_focus.py',
        ROOT / 'training/native_unknown_oe.py', ROOT / 'training/train_unified_native_short.py',
        ROOT / 'artifacts/unified-font-v4/run-native-short-v1/SELECTION.json',
        ROOT / 'artifacts/unified-font-v4/NATIVE_SHORT_COMPLETION.json',
...
def objective(model, batch):
    ...
    return replay[0] + FOCUS_WEIGHT * focus[0] + oe, {
        'unknown_oe': oe, 'unknown_oe_rows': (batch['targets'] == 24).sum(),
...
def design(args, bindings):
    return {'schema': 'flux-glyph-unified-native-oe-training-plan-v1', 'architecture': ARCHITECTURE,
...
def preflight(args):
    ...
    report = {'schema': 'flux-glyph-native-oe-gradient-preflight-v1', 'passed': True, 'steps': 4,
...
def train(args):
    ...
    protocol = {**planned, 'schema': 'flux-glyph-unified-native-oe-training-freeze-v1', 'bindings': bindings,
    ...
    selection = {**protocol, 'schema': 'flux-glyph-unified-native-oe-selection-v1', 'selected': record,
