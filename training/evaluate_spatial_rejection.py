...
from scripts.prepare_spatial_rejection_release import validate_calibration_boundary
from export_spatial_rejection import (CAL_REGIONS, METADATA_SCHEMA, PARITY_SCHEMA,
    STEPS, validate)
...
def build_development_report(selection, parity, measured, by_view, confusion, denominators,
    ...
    require(selection.get('trial') in ('rejection-low', 'rejection-medium', 'rejection-high')
    ...
    return {'schema': 'flux-glyph-spatial-rejection-development-regression-v1',
...
def evaluate(args):
    ...
    source_files = [Path(__file__), ROOT / 'training/export_spatial_rejection.py',
        ROOT / 'scripts/prepare_spatial_rejection_release.py',
    ...
    freeze = {'schema': 'flux-glyph-spatial-rejection-development-freeze-v1',
