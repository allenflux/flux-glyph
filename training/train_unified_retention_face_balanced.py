#!/usr/bin/env python3
"""Train one widened CNN with face-balanced verified WenKai TRAIN pairs."""
...
OBJECTIVE_VARIANT='wide_cnn_face_balanced_known_unknown_teacher_transfer'
DESIGN_CHANGES=[
    'Use the verified merged Regular, Light, Medium and MicroHei paired TRAIN tensor.',
    'Cycle paired WenKai Light, Medium and Regular faces exactly while retaining the fixed 96-slot replay.',
    'Keep the widened architecture, b08/R21 true-target teacher policy, loss, schedule and runtime fixed.']
...
def read_objective_plan(path, acceptance_plan_sha256, weight_teacher_cache_sha256):
    ...
    expected={'schema':'flux-glyph-wide-face-balanced-plan-v1',
...
class FullSupplementCounts(TruthCorrectCounter):
    ...
    def report(self):
        return {**super().report(),'schema':'flux-glyph-retention-face-balanced-counts-v1',
...
def validate_training_counts(report,families,steps):
    ...
    require(report.get('schema')=='flux-glyph-retention-face-balanced-counts-v1'
...
def train(args):
    ...
    paths=[Path(__file__),ROOT/'training/retention_adapter_network.py',ROOT/'training/train_unified_retention_core.py',
        ...
        ROOT/'src/flux_glyph/unified_font.py',ROOT/'src/flux_glyph/region_font.py',ROOT/'training/evaluate_unified_retention_face_balanced.py',ROOT/'training/retention_supplement_sampler.py',ROOT/'training/retention_source_balanced_sampler.py',ROOT/'training/retention_paired_known_sampler.py',ROOT/'training/train_unified_retention_source_balanced.py',
        ...
        ROOT/'training/retention_face_balanced_sampler.py',ROOT/'training/merge_unified_weight_pairs.py',
    ...
    protocol={'schema':'flux-glyph-unified-retention-wide-face-balanced-protocol-v1','architecture':ARCHITECTURE,'families':families,
    ...
    selection={**protocol,'schema':'flux-glyph-unified-retention-wide-face-balanced-selection-v1','selected':selected,'history':history,
...
def parser():
    ...
    p.add_argument('--objective-plan',type=Path,default=ROOT/'artifacts/unified-font-v3/face-balanced-plan-v1/PLAN.json')
    p.add_argument('--checkpoint',type=Path,default=ROOT/'artifacts/unified-font-v2/run-core-v1/model.pth')
    p.add_argument('--output',type=Path,default=ROOT/'artifacts/unified-font-v3/run-wide-face-balanced-v1')
