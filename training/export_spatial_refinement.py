...
PARITY_SCHEMA = 'flux-glyph-spatial-refinement-onnx-parity-v1'
METADATA_SCHEMA = 'flux-glyph-unified-region-font-v1'
PLAN_SCHEMA = 'flux-glyph-spatial-refinement-plan-v1'
...
TRIALS = ('refinement-low', 'refinement-medium', 'refinement-high')
RATES = dict(zip(TRIALS, (1.2e-4, 1.6e-4, 2.4e-4)))
REPAIR_WEIGHTS = {name: 1. for name in TRIALS}
SELECTION_POLICY = ('fixed final step only; first fully CAL-qualified trial in declared '
                    'ascending learning-rate order')
...
OBJECTIVE_SHA = '75cd10a5c91c9f641fe48e296e99c40d37808b693b86f94c52e852fa6fcac264'
...
GRID_ROOT = ROOT / 'artifacts/unified-font-v8/refinement-plans-v1'
RUN_ROOT = ROOT / 'artifacts/unified-font-v8/refinement-runs-v1'
...
def validate_loss_trace(parts, weight):
    ...
    require(all(math.isfinite(parts.get(k, math.nan)) and parts[k] >= 0 for k in names)
            ...
            and math.isclose(parts['repair_total'], .25 * parts['repair_unknown_ce'] +
                             .05 * parts['repair_kaiti_ce'] + weight * parts['repair_system_confusion'],
...
def validate(run, data, plan_path):
    ...
    require(result.get('schema') == 'flux-glyph-spatial-refinement-grid-result-v1'
    ...
    require(grid.get('schema') == 'flux-glyph-spatial-refinement-grid-v1'
    ...
    require(selection.get('schema') == 'flux-glyph-spatial-refinement-selection-v1'
            and protocol.get('schema') == 'flux-glyph-spatial-refinement-training-freeze-v1'
...
def export(args):
    ...
    source_files = [Path(__file__), ROOT/'training/evaluate_spatial_refinement.py',
        ROOT/'scripts/prepare_spatial_refinement_release.py', ROOT/'training/train_spatial_refinement.py',
        ROOT/'training/spatial_rejection_network.py', ROOT/'training/spatial_residual_network.py',
        ROOT/'training/spatial_balanced_objective.py', ROOT/'training/spatial_rejection_objective.py', ROOT/'training/spatial_region_network.py',
