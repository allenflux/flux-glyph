#!/usr/bin/env python3
"""Export one residual-head font CNN while preserving its frozen encoder and size branch."""
...
def validate(run, data):
    ...
    require(selection.get('schema') == 'flux-glyph-unified-retention-adapter-selection-v1'
    ...
    require(protocol.get('schema') == 'flux-glyph-unified-retention-adapter-protocol-v1'
    ...
    expected = {'architecture': ARCHITECTURE, 'objective_variant': trainer.OBJECTIVE_VARIANT,
        ...
        'weight_decay': 1e-4, 'gradient_clip_norm': 5., 'training_device': 'cpu', 'seed': 2026091405,
    ...
    required = [ROOT/name for name in ('training/train_unified_retention_adapter.py', 'training/retention_adapter_network.py',
        ...
        'training/evaluate_unified_retention_adapter.py', 'src/flux_glyph/unified_font.py', 'src/flux_glyph/region_font.py')]
    ...
    cache_path = source_entry(selection['cache_manifest'], bindings); cache_manifest = trainer.read(cache_path)
...
def validate_counts(run, selection):
    ...
    require(sampling['slots'] == slots and sampling['base']['known_rows'] == 96000 and sampling['base']['unknown_rows'] == 32000
            and counts['schema'] == 'flux-glyph-retention-adapter-training-counts-v1' and counts['steps'] == 2000
...
def metadata_for_export(selection, cal, parent_metadata, model_sha256, checkpoint_sha256,
    ...
    return {'schema': 'flux-glyph-unified-region-font-v1', 'algorithm': 'unified-region-cnn64x256-v1',
        ...
        **copy.deepcopy(FIXED_RUNTIME), 'font_label_groups': cal['manifest']['font_label_groups'],
        ...
        'validation': {'kind': 'frozen_encoder_residual_head_retention_calibration_only_at_export',
...
def export(args):
    ...
    sources = {str((ROOT/name).resolve()): sha(ROOT/name) for name in
        ('training/export_unified_retention_adapter.py', 'training/train_unified_retention_adapter.py',
