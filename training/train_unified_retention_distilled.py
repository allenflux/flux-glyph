#!/usr/bin/env python3
"""Supervised weight repair with a frozen R21 teacher used only during training."""
...
OBJECTIVE_VARIANT='r21_non_pingfang_correct_teacher_kl'
OBJECTIVE={**BASE_OBJECTIVE,'teacher_outputs':True,'distillation':True,'second_encoder':True,
    ...
    'teacher_mask':'TRAIN true target is not PingFang and teacher argmax equals true target',
...
def distilled_losses(logits,ratios,targets,sizes,teacher_logits,families):
    import torch
    registry(families)
    require(len(families)==25 and 'PingFang' in families and logits.shape==(BATCH_SIZE,len(families))
    ...
    mask=(targets!=families.index('PingFang')) & (teacher_logits.argmax(1)==targets)
    ...
    def update(self,rows,mask):
        ...
        for row,eligible in zip(rows,mask):
            ...
            require(row.get('split')=='train' and row.get('native_font_verified') is True
                    ...
                    and (not eligible or family!='PingFang'),'Teacher mask contains invalid or non-TRAIN evidence')
        ...
        self.steps+=1;self.eligible_per_step.append(sum(mask))
...
def validate_distillation_report(report,families,steps):
    ...
    require(report['eligible_family_rows'].get('PingFang',0)==0,'PingFang must not receive a teacher target')
...
def train(args):
    ...
    paths=[Path(__file__),ROOT/'training/train_unified_retention.py',ROOT/'training/evaluate_unified_retention_distilled.py',
    ...
    protocol={'schema':'flux-glyph-unified-retention-training-protocol-v1','architecture':ARCHITECTURE,
        ...
        'objective_variant':OBJECTIVE_VARIANT,'student_initializer':student_initializer,'teacher':teacher_identity,
    ...
    for step in range(1,STEPS+1):
        ...
        loss,ce,size_loss,kl,mask=distilled_losses(logits,ratio,targets,sizes,teacher_logits,families)
    ...
    selection={'schema':'flux-glyph-unified-retention-selection-v1','architecture':ARCHITECTURE,'policy':POLICY,
        ...
        'objective_variant':OBJECTIVE_VARIANT,'objective':OBJECTIVE,'student_initializer':student_initializer,'teacher':teacher_identity,
