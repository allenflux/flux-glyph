#!/usr/bin/env python3
"""Train the face-balanced widened CNN with one known angular-margin auxiliary."""
...
import train_unified_retention_face_balanced as facebalanced
...
OBJECTIVE_VARIANT='wide_cnn_face_balanced_known_angular_margin_teacher_transfer'
DESIGN_CHANGES=[
    'Add only a weight-0.10 angular-margin auxiliary on true-known TRAIN rows using existing style features and family-head weights.',
    'Keep the candidate-17 b08-to-wide initializer, face-balanced replay, data, teachers, base losses, schedule, architecture and runtime fixed.']
OBJECTIVE={**facebalanced.OBJECTIVE,'angular_margin':ANGULAR_MARGIN}
...
def read_objective_plan(path, acceptance_plan_sha256, weight_teacher_cache_sha256):
    ...
    expected={'schema':'flux-glyph-wide-angular-margin-plan-v1',
...
def full_losses(logits,ratios,features,family_head_weight,targets,sizes,teacher_logits,rows,families):
    import torch
    base_total,ce,size_loss,preservation,mask=facebalanced.full_losses(
        logits,ratios,targets,sizes,teacher_logits,rows,families)
...
class FullSupplementCounts(TruthCorrectCounter):
    ...
    def report(self):
        return {**super().report(),'schema':'flux-glyph-retention-angular-margin-counts-v1',
...
def validate_training_counts(report,families,steps):
    ...
    require(report.get('schema')=='flux-glyph-retention-angular-margin-counts-v1'
...
def train(args):
    ...
    paths=[Path(__file__),ROOT/'training/retention_adapter_network.py',ROOT/'training/train_unified_retention_core.py',
        ...
        ROOT/'src/flux_glyph/unified_font.py',ROOT/'src/flux_glyph/region_font.py',ROOT/'training/evaluate_unified_retention_margin.py',ROOT/'training/retention_supplement_sampler.py',ROOT/'training/retention_source_balanced_sampler.py',ROOT/'training/retention_paired_known_sampler.py',ROOT/'training/train_unified_retention_source_balanced.py',
    ...
    protocol={'schema':'flux-glyph-unified-retention-wide-angular-margin-protocol-v1','architecture':ARCHITECTURE,'families':families,
    ...
    for step in range(1,STEPS+1):
        ...
        if step%100==0:print(json.dumps({'step':step,'loss':float(loss.detach()),'ce':float(ce.detach()),'teacher_kl':float(preservation.detach()),'size_loss':float(size_loss.detach()),'angular_margin_weighted_loss':float(auxiliary.detach()),'seconds':round(time.monotonic()-started,1)}),flush=True)
    ...
    selection={**protocol,'schema':'flux-glyph-unified-retention-wide-angular-margin-selection-v1','selected':selected,'history':history,
...
def parser():
    ...
    p.add_argument('--objective-plan',type=Path,default=ROOT/'artifacts/unified-font-v3/angular-margin-plan-v1/PLAN.json')
    p.add_argument('--checkpoint',type=Path,default=ROOT/'artifacts/unified-font-v2/run-core-v1/model.pth')
    p.add_argument('--output',type=Path,default=ROOT/'artifacts/unified-font-v3/run-wide-angular-margin-v1')
