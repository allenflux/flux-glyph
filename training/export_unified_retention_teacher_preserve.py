#!/usr/bin/env python3
"""Export one trained CNN with its exact same-initializer b08 TRAIN teacher provenance."""
...
def validate(run, data):
    ...
    require(selection.get('schema') == 'flux-glyph-unified-retention-teacher-preserve-selection-v1'
    ...
    require(protocol.get('schema') == 'flux-glyph-unified-retention-teacher-preserve-protocol-v1'
    ...
    expected = {'architecture': ARCHITECTURE, 'objective_variant': trainer.OBJECTIVE_VARIANT,
        ...
        'weight_decay': 1e-4, 'gradient_clip_norm': 5., 'seed': 2026091412,
    ...
    required = [ROOT/name for name in ('training/train_unified_retention_teacher_preserve.py', 'training/cache_unified_student_teacher.py',
        ...
        'training/evaluate_unified_retention_teacher_preserve.py', 'src/flux_glyph/unified_font.py', 'src/flux_glyph/region_font.py')]
...
def metadata_for_export(selection, cal, parent_metadata, model_sha256, checkpoint_sha256,
    ...
    return {'schema': 'flux-glyph-unified-region-font-v1', 'algorithm': 'unified-region-cnn64x256-v1',
        ...
        **copy.deepcopy(FIXED_RUNTIME), 'font_label_groups': cal['manifest']['font_label_groups'],
        ...
        'validation': {'kind': 'full_cnn_truth_correct_b08_teacher_preservation_calibration_only_at_export',
...
def export(args):
    ...
    sources = {str((ROOT/name).resolve()): sha(ROOT/name) for name in
        ('training/export_unified_retention_teacher_preserve.py', 'training/train_unified_retention_teacher_preserve.py',
