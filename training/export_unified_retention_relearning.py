#!/usr/bin/env python3
"""Export one wide CNN trained with asymmetric named/unknown teacher preservation."""
...
def validate_objective_plan(selection, retention_plan_path):
    ...
    import train_unified_retention_relearning as trainer
...
def validate_fixed_final_selection(selection):
    ...
    require([row['step'] for row in history] == [6000]
            ...
            'Angular-margin selection must be the sole fixed final checkpoint')
    return history
...
def validate(run, data):
    ...
    require(selection.get('schema') == 'flux-glyph-unified-retention-wide-named-relearning-selection-v1'
            and selection.get('architecture') == ARCHITECTURE and selection.get('promotion_allowed') is True
            and not (run/'NO_PROMOTABLE_CHECKPOINT.json').exists(), 'Named-relearning run has no promotable frozen checkpoint')
    protocol = trainer.read(run/'TRAINING_FREEZE.json')
    require(protocol.get('schema') == 'flux-glyph-unified-retention-wide-named-relearning-protocol-v1'
            ...
            'Named-relearning protocol and selection differ')
    require(all(key not in document for document in (selection, protocol)
            ...
            'Obsolete unknown supervision metadata cannot describe named-relearning training')
    expected = {'architecture': ARCHITECTURE, 'objective_variant': trainer.OBJECTIVE_VARIANT,
        ...
        'asymmetric_teacher_preservation': trainer.ASYMMETRIC_PRESERVATION,
        'teacher_distribution_kl': False,
        'teacher_distribution_kl_all_eligible_rows': False,
        'teacher_distribution_kl_scope': trainer.OBJECTIVE['teacher_distribution_kl_scope'],
    ...
    validate_asymmetric_teacher_provenance(selection)
    for field in ('initial_parameter_groups_sha256', 'selected_parameter_groups_sha256', 'final_parameter_groups_sha256'):
    ...
    require(base.get('schema') == 'flux-glyph-unified-retention-selection-v1'
            ...
            'Named-relearning base state, classes or source closure differ')
    ...
    require(base['retention_plan']['sha256'] == sha(plan_path), 'Named-relearning retention conditions changed')
    ...
    require(cal['manifest_sha256'] == selection['data_manifest_sha256'] == base['data_manifest_sha256']
            and cal['families'] == selection['families'] == plan['families'], 'Named-relearning data root or class order differs')
    for key in ('parent_metadata', 'parent_checkpoint', 'parent_selection_sha256'):
        require(selection[key] == base[key], 'Named-relearning parent provenance differs')
    parent_metadata = source_entry(selection['parent_metadata'], bindings)
    required = [ROOT/name for name in ('training/train_unified_retention_relearning.py', 'training/retention_asymmetric_teacher_loss.py',
        'training/train_unified_retention_margin.py',
        ...
        'training/evaluate_unified_retention_relearning.py', 'src/flux_glyph/unified_font.py', 'src/flux_glyph/region_font.py')]
    ...
    for split in ('train', 'calibration'):
        ...
        require(part['split'] == split and part['families'] == selection['families']
                and part['root_manifest_sha256'] == cal['manifest_sha256'], 'Named-relearning source partition differs')
        for name in ('metadata', 'array'):
            path = (data/split/part[name]['path']).resolve()
            require(path.parent == data/split and bindings.get(str(path)) == part[name]['sha256'], 'Named-relearning requested data path is unbound')
    ...
    require(all(bindings.get(str(path.resolve())) == sha(path) for path in required), 'Missing exact named-relearning source binding')
    ...
    require(set(selection['baseline_bindings']) == {'BASELINE.json', 'BASELINE_CALIBRATION_DECISIONS.json',
            ...
            'Named-relearning baseline does not reproduce the frozen base cache')
    ...
    for record in history:
        directory = f"checkpoints/step{record['step']:05d}"
        require(set(record['artifacts']) == {'checkpoint', 'outputs', 'decisions', 'metrics'}, 'Incomplete named-relearning step evidence')
        ...
        require(actual == strip_artifacts(record) == trainer.read(paths['metrics']), 'Named-relearning CAL metrics do not reproduce cached outputs')
    for key, name, field in [('outputs', 'CALIBRATION_OUTPUTS.npz', 'calibration_outputs_sha256'),
                            ('decisions', 'CALIBRATION_DECISIONS.json', 'calibration_decisions_sha256')]:
        require(sha(run/name) == selection[field] == selection['selected']['artifacts'][key]['sha256'], 'Selected named-relearning CAL bytes differ')
...
def validate_counts(run, selection, data):
    ...
    require(all(key not in document for document in (counts, sampling)
                for key in ('soft_unknown_supervision', 'unknown_binary_supervision')),
            'Obsolete unknown supervision metadata in named-relearning evidence')
...
def validate_asymmetric_teacher_provenance(selection):
    """Require named confidence-floor preservation and unknown-only full KL."""
    import train_unified_retention_relearning as trainer
    from retention_asymmetric_teacher_loss import ASYMMETRIC_PRESERVATION as canonical
    spec = selection.get('asymmetric_teacher_preservation')
    objective = selection.get('objective', {})
    require(spec == canonical == trainer.ASYMMETRIC_PRESERVATION
            and objective == trainer.OBJECTIVE
            and objective.get('asymmetric_teacher_preservation') == canonical
            and objective.get('teacher_preservation_weight') == canonical['outer_weight']
            and canonical.get('returned_loss_includes_outer_weight') is False
            and canonical.get('eligibility') == objective.get('teacher_mask')
            and canonical.get('teacher_frozen') is True
            and canonical.get('training_scope') == 'TRAIN only'
            and canonical.get('confidence_threshold') is None
            and canonical.get('adds_parameters') is False
            and canonical.get('inference_rule') is False
            and canonical.get('deployed_operator') is False,
            'Asymmetric teacher preservation is not the canonical TRAIN-only objective')
    require(selection.get('teacher_distribution_kl') is False
            and selection.get('teacher_distribution_kl_all_eligible_rows') is False
            and selection.get('teacher_distribution_kl_scope') == canonical['distribution_kl_scope']
            and selection.get('named_teacher_distribution_kl') is False
            and selection.get('unknown_teacher_distribution_kl') is True
            and selection.get('named_teacher_confidence_floor') is True
            and objective.get('teacher_distribution_kl') is False
            and objective.get('teacher_distribution_kl_all_eligible_rows') is False
            and objective.get('teacher_distribution_kl_scope') == canonical['distribution_kl_scope']
            and objective.get('named_teacher_distribution_kl') is False
            and objective.get('unknown_teacher_distribution_kl') is True
            and objective.get('named_teacher_confidence_floor') is True,
            'Teacher distribution KL must be unknown-only and named preservation must be a confidence floor')
    return spec
...
def training_metadata(selection):
    ...
    keys = ('angular_margin', 'angular_margin_applies_at_inference',
        'angular_margin_adds_deployment_parameters', 'asymmetric_teacher_preservation',
        'teacher_distribution_kl', 'teacher_distribution_kl_all_eligible_rows',
        'teacher_distribution_kl_scope', 'named_teacher_distribution_kl',
        'unknown_teacher_distribution_kl', 'named_teacher_confidence_floor',
...
def metadata_for_export(selection, cal, parent_metadata, model_sha256, checkpoint_sha256,
    ...
    return {'schema': 'flux-glyph-unified-region-font-v1', 'algorithm': 'unified-region-cnn64x256-v1',
        ...
        **copy.deepcopy(FIXED_RUNTIME), 'font_label_groups': cal['manifest']['font_label_groups'],
        ...
        'validation': {'kind': 'full_cnn_training_named_relearning_calibration_only_at_export',
...
def build_parity_report(selection, selection_sha256, checkpoint_sha256, model_sha256, metadata_sha256,
    ...
    'asymmetric_teacher_preservation': copy.deepcopy(selection['asymmetric_teacher_preservation']),
    'teacher_distribution_kl': False,
    'teacher_distribution_kl_all_eligible_rows': False,
    'teacher_distribution_kl_scope': selection['teacher_distribution_kl_scope'],
    'named_teacher_distribution_kl': False, 'unknown_teacher_distribution_kl': True,
    'named_teacher_confidence_floor': True,
...
def export(args):
    ...
    require(not output.exists() and not (run/'PARITY.json').exists(), 'Preserve earlier named-relearning export attempts')
    ...
    sources = {str((ROOT/name).resolve()): sha(ROOT/name) for name in
        ('training/export_unified_retention_relearning.py', 'training/train_unified_retention_relearning.py', 'training/retention_asymmetric_teacher_loss.py',
        'training/train_unified_retention_margin.py',
         'training/retention_dual_teacher_loss.py', 'training/retention_confidence_floor_loss.py',
        'training/retention_angular_margin_loss.py', 'training/train_unified_retention_face_balanced.py',
         'training/train_unified_retention_confidence_floor.py',
         'training/evaluate_unified_retention_relearning.py', 'training/wide_region_network.py',
         ...
         'training/export_unified_retention_margin.py',
    ...
    require(step['architecture'] == ARCHITECTURE and step['families'] == selection['families']
            ...
            and state_sha(step['state_dict']) == selection['state_after_sha256'], 'Final named-relearning state differs from the selected saved checkpoint')
    ...
    require(replace_groupnorm(converted, high_precision=True) == 4
            and state_sha(converted.state_dict()) == selection['state_after_sha256'], 'GroupNorm lowering changed named-relearning parameters')
    ...
    for count in (1, 7, 32, 128):
        ...
        require(out.dtype == size.dtype == np.float32 and out.shape == (len(samples), 25) and size.shape == (len(samples),)
                and np.isfinite(out).all() and np.isfinite(size).all(), 'Invalid dynamic named-relearning model output')
    ...
    require(validate(run, data) == selection and all(sha(path) == digest for path, digest in {**sources, **evidence}.items())
            ...
            'Named-relearning evidence or weights changed during export')
