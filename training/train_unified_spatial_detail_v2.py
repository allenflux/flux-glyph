#!/usr/bin/env python3
"""Train one spatial-detail CNN with the frozen native-pairs loss and samples."""
...
from cache_r22_train_teacher import (BASE_RUN, BASE_SHA, BASE_SELECTION_SHA, BASE_STATE_SHA,
    SOURCES, load_base, load_training, read, write_new, source_bindings)
from prepare_unified_regions import FAMILIES, load_split
from spatial_region_network import ARCHITECTURE, expand_spatial_pool
...
from native_font_pairs import (POLICY as PAIR_POLICY, SAMPLING as PAIR_SAMPLING,
    build_pairs, NativeFontPairSampler, pair_margin_loss)
...
PAIR_RECONSTRUCTION = ROOT / 'artifacts/unified-font-v4/native-pairs-reconstruction-v1/REPORT.json'
...
PAIR_COUNT = 16
PAIR_ROWS = 32
PAIR_WEIGHT = PAIR_POLICY['weight']
...
OBJECTIVE = {
    ...
    'pair_policy': PAIR_POLICY, 'pair_sampling': PAIR_SAMPLING,
    'pair_batch': PAIR_COUNT, 'pair_rows': PAIR_ROWS,
    'cnn_input_rows_per_step': 180, 'one_cnn_forward_per_step': True,
    'pair_loss': '.1 * mean softplus(1 - [(logit_a[ya] - logit_a[yb]) - (logit_b[ya] - logit_b[yb])])',
    'pair_rows_have_ce': False, 'pair_rows_have_size_loss': False,
    'pair_class_bias_cancels': True,
    'rationale': 'Keep native-mobile supervision exact and add a small matched-content font discrimination margin.',
    'replay_rng_unchanged': True, 'focus_rng_independent': True, 'mobile_rng_independent': True,
    'pair_rng_independent': True,
...
def validate_pair_contract():
    require(PAIR_POLICY == {
        'schema': 'flux-glyph-native-font-pair-margin-v1', 'pairs_per_batch': 16,
        ...
        and PAIR_SAMPLING.get('pairs_per_batch') == PAIR_COUNT
        and PAIR_SAMPLING.get('rng_independent') is True
        and PAIR_SAMPLING.get('platform_features') is False
        and PAIR_SAMPLING.get('predictions_used_as_labels') is False
        and PAIR_SAMPLING.get('old_mobile_ce_pool_changed') is False,
        'Native-font pair policy, loss, or isolation contract differs')
    return PAIR_POLICY


def validate_pair_proof(proof):
    helper = str((ROOT / 'training/native_font_pairs.py').resolve())
    require(proof.get('schema') == 'flux-glyph-native-font-pair-pools-v1'
            and proof.get('sampling') == PAIR_SAMPLING and proof.get('policy') == PAIR_POLICY
            and proof.get('native_identities') == 1630
            and proof.get('pair_eligible_native_identities') == 1509
            and proof.get('unpaired_native_identities') == 121
            and proof.get('unpaired_remain_in_original_mobile_ce') is True
            and proof.get('cohorts') == 124
            and proof.get('cohorts_with_different_targets') == 93
            and proof.get('source_slot_count') == 144
            and isinstance(proof.get('eligible_cohorts_by_slot_counterpart'), dict)
            and len(proof['eligible_cohorts_by_slot_counterpart']) == 2424
            and isinstance(proof.get('native_identity_tile_proof_sha256'), str)
            and len(proof['native_identity_tile_proof_sha256']) == 64
            and proof.get('bindings') == {helper: sha(helper)}
            and all(proof.get(key) is False for key in
                    ('model_inference', 'calibration_read', 'development_read', 'test_read')),
            'Native-font pair source proof, coverage, or isolation differs')
    return proof
...
def files_bound(args, datasets):
    ...
    files = [Path(__file__), ROOT / 'training/spatial_region_network.py',
        ROOT / 'training/train_unified_spatial_detail.py',
        ROOT / 'training/train_unified_native_pairs.py', PRIOR_COUNTS,
        ROOT / 'training/native_mobile_short_training.py',
        ROOT / 'training/native_font_pairs.py',
        ...
        ROOT / 'artifacts/unified-font-v4/native-pairs-reconstruction-v1/REPORT.json',
        ROOT / 'artifacts/unified-font-v4/reconstruct_native_pairs.py',
        ROOT / 'artifacts/unified-font-v4/native-mobile-pairing-audit-v1/REPORT.json',
        ROOT / 'artifacts/unified-font-v4/native-mobile-pairing-audit-v1/audit.py',
        ROOT / 'artifacts/unified-font-v4/run-native-mobile-v1/SELECTION.json',
        ROOT / 'artifacts/unified-font-v4/NATIVE_MOBILE_COMPLETION.json',
        ROOT / 'artifacts/unified-font-v4/native-mobile-comparison-v1/REPORT.json',
        ROOT / 'artifacts/unified-font-v4/native-mobile-train-fit-v1/REPORT.json',
        ROOT / 'artifacts/unified-font-v4/native-mobile-train-fit-canonical-v1/REPORT.json',
...
def context(args):
    ...
    require(sha(PRIOR_COUNTS) == PRIOR_COUNTS_SHA, 'Prior native-pairs sampling record changed')
    ...
    source, base_selection = load_base()
    model, transfer = expand_spatial_pool(source)
    ...
    pair_pools, pair_proof = build_pairs(mobile)
    validate_pair_proof(pair_proof)
    pair_reconstruction = read(PAIR_RECONSTRUCTION)
    require(pair_reconstruction.get('schema') == 'flux-glyph-native-font-pair-reconstruction-v1'
            and pair_reconstruction.get('seed') == SEED + 3
            and pair_reconstruction.get('helper_sha256') == sha(ROOT / 'training/native_font_pairs.py')
            and pair_reconstruction.get('source_proof') == pair_proof
            and all(pair_reconstruction.get(key) is False for key in
                    ('model_inference', 'calibration_read', 'development_read', 'test_read')),
            'Independent native-font pair sampler reconstruction differs')
    pair_sampler = NativeFontPairSampler(pair_pools, SEED + 3)
    transfer = verify_transfer(source, model, datasets, mobile, transfer)
    del source
    return (model, datasets, replay, sampler, focus_sampler, mobile, mobile_sampler, base_selection,
            ...
            pair_sampler, pair_proof, pair_reconstruction, transfer)


def horizontal_spread(model):
    """At transfer each old horizontal coefficient has four equal children."""
    weight = model.style[0].weight.detach().reshape(384, 128, 4, 4, 4)
    return float((weight - weight[..., :1]).abs().max())


def verify_transfer(source, model, datasets, mobile, transfer):
    """Check initial behavior on deterministic actual TRAIN tiles, outside RNG streams."""
    import hashlib
    import torch
    arrays = {'replay': datasets['original']['tiles'],
              'ios_short': mobile['datasets']['ios']['tiles'],
              'android_short': mobile['datasets']['android']['tiles']}
    indices = {key: np.linspace(0, len(value) - 1, 8, dtype=np.int64).tolist()
               for key, value in arrays.items()}
    pixels = np.concatenate([value[indices[key]] for key, value in arrays.items()])
    images = torch.from_numpy(pixels)
    source_dtype = next(source.parameters()).dtype
    model_dtype = next(model.parameters()).dtype
    before = state_sha(source.state_dict())
    source.eval(); model.eval()
    with torch.no_grad():
        old_font, old_size = source(images)
        new_font, new_size = model(images)
    font_error = float((old_font - new_font).abs().max())
    size_error = float((old_size - new_size).abs().max())
    # Wider reductions accumulate float32 roundoff differently. Verify the
    # transfer identity independently in float64, then cap float32 absolute
    # error below the existing export parity budget (without changing it).
    try:
        source.double(); model.double()
        with torch.no_grad():
            old_font64, old_size64 = source(images.double())
            new_font64, new_size64 = model(images.double())
        font_error64 = float((old_font64 - new_font64).abs().max())
        size_error64 = float((old_size64 - new_size64).abs().max())
    finally:
        source.to(dtype=source_dtype); model.to(dtype=model_dtype)
    require(font_error64 <= 1e-10 and size_error64 <= 1e-10
            and torch.equal(old_font64.argmax(1), new_font64.argmax(1))
            and torch.isfinite(new_font).all()
            and torch.allclose(old_size, new_size, rtol=1e-5, atol=1e-5)
            and font_error <= 1e-4 and size_error <= 1e-5
            and torch.equal(old_font.argmax(1), new_font.argmax(1)),
            'Spatial transfer changed initial TRAIN font or size predictions')
    require(before == state_sha(source.state_dict()) == BASE_STATE_SHA
            and horizontal_spread(model) == 0., 'Spatial transfer mutated source or coefficient inheritance')
    return {**transfer, 'initial_dtype': str(source_dtype), 'source_state_sha256': before,
            'initial_state_sha256': state_sha(model.state_dict()),
            'initial_parity': {'verified': True, 'device': 'cpu', 'train_tiles': 24,
                'indices': indices, 'input_sha256': hashlib.sha256(pixels.tobytes()).hexdigest(),
                'maximum_logit_error': font_error, 'maximum_size_error': size_error,
                'float32_logit_absolute_limit': 1e-4, 'float32_size_absolute_limit': 1e-5,
                'float64_maximum_logit_error': font_error64, 'float64_maximum_size_error': size_error64,
                'float64_absolute_limit': 1e-10, 'release_parity_limits_changed': False,
                'all_argmax_equal': True, 'source_unchanged': True,
                'sampler_rng_used': False, 'calibration_read': False,
                'development_read': False, 'test_read': False}}


def _pair_identity(row):
    return row['short_dataset'], row['source_id'], row['region_id']


def _opaque_rgb(value):
    value = value.upper()
    require(value.startswith('#') and len(value) in (7, 9)
            and (len(value) == 7 or value.endswith('FF')), 'Pair text color must be opaque RGB')
    try:
        int(value[1:], 16)
    except ValueError as error:
        raise ValueError('Pair text color must be opaque RGB') from error
    return value[:7]


def validate_pair_rows(pairs):
    require(isinstance(pairs, list) and len(pairs) == PAIR_COUNT,
            'Pair sampler must return exactly 16 row pairs')
    flat = []
    for pair in pairs:
        require(isinstance(pair, tuple) and len(pair) == 2, 'Pair sampler output shape differs')
        a, b = pair
        require(a['target'] != b['target']
                and all(isinstance(row['target'], int) and 0 <= row['target'] < len(FAMILIES)
                        and row['family'] == FAMILIES[row['target']]
                        and row['split'] == 'train' and row['native_font_verified'] is True
                        for row in pair)
                and _pair_identity(a) != _pair_identity(b)
                and a['text'] == b['text']
                and re.fullmatch(r'[0-9a-f]{64}', a['normalized_text_sha256']) is not None
                and a['normalized_text_sha256'] == b['normalized_text_sha256']
                and a['script'] in ('han', 'latin', 'numeric')
                and b['script'] in ('han', 'latin', 'numeric')
                and ('han' if a['script'] == 'han' else 'latin')
                    == ('han' if b['script'] == 'han' else 'latin')
                and a['glyph_count'] == b['glyph_count']
                and float(a['native_font_size_px']) == float(b['native_font_size_px'])
                and _opaque_rgb(a['text_color_hex']) == _opaque_rgb(b['text_color_hex'])
                and a['view'] == b['view']
                and all(row['tile_count'] == 1 and row['short_dataset'] in ('ios', 'android')
                        for row in pair),
                'Native-font pair truth, content, style, view, or target differs')
        flat.extend(pair)
    require(len(flat) == PAIR_ROWS
            and all(flat[index] is pairs[index // 2][index % 2] for index in range(PAIR_ROWS)),
            'Pair rows must remain interleaved a0,b0,a1,b1')
    return flat


def inputs(replay, sampler, focus_sampler, mobile, mobile_sampler, pair_sampler, device):
    import torch
    rows = sampler.batch()
    ...
    pair_rows = validate_pair_rows(pair_sampler.batch())
    px = np.stack([mobile['datasets'][r['short_dataset']]['tiles'][r['tile_start']]
                   for r in pair_rows])
    tensor = lambda values: torch.from_numpy(np.asarray(values)).to(device)
    return {'images': tensor(np.concatenate((images, fx[:32], mx, px))), 'teacher': tensor(teacher),
        ...
        'pair_rows': pair_rows,
        'pair_targets': tensor(np.array([r['target'] for r in pair_rows], np.int64)),
...
def objective(model, batch):
    ...
    require(font.shape == (180, 25) and size.shape == (180,) and features.shape[0] == 180,
    ...
    pair_loss = pair_margin_loss(font[148:180], batch['pair_targets'])
    pair_weighted = PAIR_WEIGHT * pair_loss
    return replay[0] + FOCUS_WEIGHT * focus[0] + OE_MULTIPLIER * oe + mobile_loss + pair_weighted, {
        ...
        'pair_count': pair_loss.new_tensor(PAIR_COUNT), 'pair_rows': pair_loss.new_tensor(PAIR_ROWS)}


def design(args, bindings, transfer):
    return {'schema': 'flux-glyph-unified-spatial-detail-training-plan-v1', 'architecture': ARCHITECTURE,
            'spatial_transfer': transfer, 'controlled_change': '4x4 to 4x16 pooling and inherited first style linear map only',
            'model_graph_changed': True, 'runtime_policy_changed': False,
            'optimization_limitation': 'More spatial coefficients change parameterization and optimizer dynamics even at the same nominal learning rate; not a causal separation of detail from capacity.',
            'reference_trial': 'native-pairs-v1', 'prior_training_counts_sha256': PRIOR_COUNTS_SHA,
            ...
            'pair_policy': PAIR_POLICY, 'pair_sampling': PAIR_SAMPLING,
            'expected_pair_budget': {'pairs': STEPS * PAIR_COUNT, 'rows': STEPS * PAIR_ROWS},
...
def _require_pair_budget(report, reconstructed, steps):
    require(report == reconstructed
            and report.get('schema') == PAIR_SAMPLING.get('schema')
            and report.get('steps') == steps
            and report.get('pairs') == steps * PAIR_COUNT
            and report.get('rows') == steps * PAIR_ROWS
            and all(isinstance(report.get(key), dict) and report[key]
                    for key in ('anchor_by_source', 'counterpart_by_source', 'rows_by_source',
                                'rows_by_family', 'pairs_by_source', 'anchor_by_script_length',
                                'pairs_by_view', 'rows_by_identity', 'rows_by_face')),
            'Native-font pair trajectory or complete accounting differs')
    return report
...
def preflight(args):
    ...
    (model, datasets, replay, sampler, focus_sampler, mobile, mobile_sampler, _, _, focus_proof,
     mobile_proof, reconstruction, pair_sampler, pair_proof, pair_reconstruction, transfer) = context(args)
    ...
    for _ in range(4):
        batch = inputs(replay, sampler, focus_sampler, mobile, mobile_sampler, pair_sampler, args.device)
        ...
        require(int(parts['pair_count'].detach()) == PAIR_COUNT
                and int(parts['pair_rows'].detach()) == PAIR_ROWS
                and bool(torch.isclose(parts['pair_weighted'], PAIR_WEIGHT * parts['pair_loss'])),
                'Pair preflight count or loss weight differs')
    ...
    pair_report = pair_sampler.report()
    ...
    _require_pair_budget(pair_report, pair_reconstruction['preflight'], 4)
    ...
    spatial_spread = horizontal_spread(model)
    require(spatial_spread > 0., 'Finer horizontal coefficients did not learn independently')
    ...
    report = {'schema': 'flux-glyph-spatial-detail-gradient-preflight-v1', 'passed': True, 'steps': 4,
              'device': args.device, 'spatial_transfer': transfer, 'horizontal_coefficient_spread': spatial_spread, 'replay_tiles': 384, 'focus_tiles': 128, 'mobile_tiles': 80,
              'pair_count': 64, 'pair_rows': 128, 'traces': traces,
              ...
              'native_font_pair_proof': pair_proof, 'pair_policy': PAIR_POLICY,
              'pair_sampling': pair_report,
    ...
    planned = {**design(args, bindings, transfer), 'preflight': {'path': str(args.output), 'sha256': sha(args.output)}}
...
def train(args):
    ...
    (model, datasets, replay, sampler, focus_sampler, mobile, mobile_sampler, base_selection,
     teacher_proof, focus_proof, mobile_proof, reconstruction,
     pair_sampler, pair_proof, pair_reconstruction, transfer) = context(args)
    bindings = files_bound(args, datasets)
    planned = read(args.plan)
    require(all(planned.get(k) == v for k, v in design(args, bindings, transfer).items()), 'Training plan changed after preflight')
    pre = planned['preflight']; proof = read(pre['path'])
    require(sha(pre['path']) == pre['sha256'] and proof['passed'] is True
            and proof['bindings'] == bindings and proof['test_read'] is False
            and proof.get('spatial_transfer') == transfer
            and proof.get('horizontal_coefficient_spread', 0.) > 0.
            and proof.get('native_font_pair_proof') == pair_proof
            and proof.get('pair_policy') == PAIR_POLICY
            and proof.get('pair_sampling') == pair_reconstruction['preflight'],
            'TRAIN gradient preflight differs')
    bindings[str(args.plan)] = sha(args.plan); bindings[pre['path']] = pre['sha256']
    before = parameter_groups(model.state_dict())
    args.output.mkdir(parents=True)
    protocol = {**planned, 'schema': 'flux-glyph-unified-spatial-detail-training-freeze-v1', 'bindings': bindings,
                ...
                'mobile_short_proof': mobile_proof, 'native_font_pair_proof': pair_proof,
    ...
    for step in range(1, STEPS + 1):
        ...
        require(int(parts['pair_count'].detach()) == PAIR_COUNT
                and int(parts['pair_rows'].detach()) == PAIR_ROWS
                and bool(torch.isclose(parts['pair_weighted'], PAIR_WEIGHT * parts['pair_loss'])),
                'Pair optimizer count or loss weight differs')
    ...
    _require_pair_budget(pair_report, pair_reconstruction['full'], STEPS)
    ...
    training_counts = {'optimizer_steps': STEPS, 'replay_rows': sum(counts.values()),
        ...
        'pair_count': STEPS * PAIR_COUNT, 'pair_rows': STEPS * PAIR_ROWS,
        'pair_policy': PAIR_POLICY, 'pair_sampling': pair_report,
    ...
    require(sha(PRIOR_COUNTS) == PRIOR_COUNTS_SHA and training_counts == read(PRIOR_COUNTS),
            'Spatial trial changed the frozen native-pairs data or sampling trajectory')
    require(horizontal_spread(model) > 0., 'Spatial coefficients remained tied after training')
    ...
    selection = {**protocol, 'schema': 'flux-glyph-unified-spatial-detail-selection-v1', 'selected': record,
