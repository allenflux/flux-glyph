...
from scripts.prepare_spatial_refinement_release import validate_calibration_boundary
from export_spatial_refinement import (CAL_REGIONS, METADATA_SCHEMA, PARITY_SCHEMA,
    STEPS, validate)
...
def build_development_report(selection, parity, measured, by_view, confusion, denominators,
    ...
    require(selection.get('trial') in ('refinement-low', 'refinement-medium', 'refinement-high')
            and parity.get('trial') == selection.get('trial')
    ...
    return {'schema': 'flux-glyph-spatial-refinement-development-regression-v1',
        ...
        'source_version_summaries': {
            ...
            'candidate': {'trial_id': selection['trial'],
...
def evaluate(args):
    ...
    source_files = [Path(__file__), ROOT / 'training/export_spatial_refinement.py',
        ROOT / 'scripts/prepare_spatial_refinement_release.py',
    ...
    freeze = {'schema': 'flux-glyph-spatial-refinement-development-freeze-v1',
