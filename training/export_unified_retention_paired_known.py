#!/usr/bin/env python3
"""Export the paired-known full CNN with separate student and frozen teacher provenance."""
...
def validate_student_initializer(selection):
    """Keep the completed source-balanced student distinct from core cached targets."""
    ...
    require(all(selection[key] == previous[key] for key in ('families', 'base_checkpoint', 'base_selection',
        ...
        and previous['initial_state_sha256'] == selection['cached_teacher_state_sha256'] == selection['base_state_sha256']
        and selection['initial_state_sha256'] != selection['cached_teacher_state_sha256']
        and all(bindings.get(path) == digest for path, digest in previous['bindings'].items()),
        'Student initialization was confused with the frozen core teacher or source closure')
...
def validate(run, data):
    ...
    require(selection.get('schema') == 'flux-glyph-unified-retention-paired-known-selection-v1'
    ...
    require(protocol.get('schema') == 'flux-glyph-unified-retention-paired-known-protocol-v1'
    ...
    expected = {'architecture': ARCHITECTURE, 'objective_variant': trainer.OBJECTIVE_VARIANT,
        ...
        'weight_decay': 1e-4, 'gradient_clip_norm': 5., 'seed': 2026091411,
    ...
    required = [ROOT/name for name in ('training/train_unified_retention_paired_known.py', 'training/retention_paired_known_sampler.py',
        ...
        'training/evaluate_unified_retention_paired_known.py', 'src/flux_glyph/unified_font.py', 'src/flux_glyph/region_font.py')]
...
def validate_known_supplement(selection, original_rows, unknown_rows):
    """The paired known images use core teacher tiles and retain their named targets."""
    import train_unified_retention_paired_known as trainer
    bindings = selection['bindings']
    ...
    require(sha(data_path) == trainer.KNOWN_MANIFEST_SHA and sha(cache_path) == trainer.KNOWN_CACHE_SHA,
            'Paired known data or cached teacher differs from the frozen capture')
    ...
    require(identity['data_manifest'] == selection['known_data'] and identity['base_checkpoint'] == selection['base_checkpoint']
            and identity['base_selection'] == selection['base_selection']
            and identity['base_state_sha256'] == selection['base_state_sha256'] == selection['cached_teacher_state_sha256']
            and identity['old_cache_manifest'] == selection['cache_manifest']
            and identity['families'] == selection['families'] and identity['partition']['split'] == 'train'
            and all(bindings.get(path) == digest for path, digest in identity['bindings'].items()),
            'Paired known cache used the student or a different teacher, registry or source')
    for name in ('data_manifest', 'preparation_freeze', 'partition_manifest', 'old_cache_manifest', 'old_cache_freeze'):
        source_entry(identity[name], bindings)
    source_entry({'path': str(cache_path.parent/'CACHE_FREEZE.json'), 'sha256': cache['cache_freeze_sha256']}, bindings)
    for item in cache['partitions']['train'].values():
...
def training_metadata(selection):
    keys = ('objective_variant', 'objective', 'unknown_binary_supervision', 'sampling', 'unknown_source_order',
        ...
        'student_initializer', 'training_data_counts',
...
def metadata_for_export(selection, cal, parent_metadata, model_sha256, checkpoint_sha256,
    ...
    return {'schema': 'flux-glyph-unified-region-font-v1', 'algorithm': 'unified-region-cnn64x256-v1',
        ...
        **copy.deepcopy(FIXED_RUNTIME), 'font_label_groups': cal['manifest']['font_label_groups'],
        ...
        'validation': {'kind': 'full_cnn_paired_known_binary_unknown_calibration_only_at_export',
...
def export(args):
    ...
    sources = {str((ROOT/name).resolve()): sha(ROOT/name) for name in
        ('training/export_unified_retention_paired_known.py', 'training/train_unified_retention_paired_known.py',
    ...
    require(state_sha(base['state_dict']) == selection['cached_teacher_state_sha256'], 'Frozen core teacher state differs')
    ...
    require(initial.get('families') == selection['families'] and initial.get('architecture') == ARCHITECTURE
            ...
            != selection['cached_teacher_state_sha256'], 'Student initialization is not the exact prior rank-selected CNN')
    for name in GROUPS:
