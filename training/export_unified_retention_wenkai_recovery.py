#!/usr/bin/env python3
"""Export one wide CNN trained with a true-WenKai TRAIN CE recovery weight."""
...
def validate_objective_plan(selection, retention_plan_path):
    ...
    import train_unified_retention_wenkai_recovery as trainer
...
def validate(run, data):
    ...
    require(selection.get('schema') == 'flux-glyph-unified-retention-wide-wenkai-recovery-selection-v1'
            and selection.get('architecture') == ARCHITECTURE and selection.get('promotion_allowed') is True
            and not (run/'NO_PROMOTABLE_CHECKPOINT.json').exists(), 'WenKai-recovery run has no promotable frozen checkpoint')
    protocol = trainer.read(run/'TRAINING_FREEZE.json')
    require(protocol.get('schema') == 'flux-glyph-unified-retention-wide-wenkai-recovery-protocol-v1'
    ...
    required = [ROOT/name for name in ('training/train_unified_retention_wenkai_recovery.py',
        ...
        'training/evaluate_unified_retention_wenkai_recovery.py',
...
def validate_wenkai_recovery_counts(counts, selection, steps=6000):
    ...
    require(counts.get('schema') == 'flux-glyph-retention-wenkai-recovery-counts-v1'
...
def metadata_for_export(selection, cal, parent_metadata, model_sha256, checkpoint_sha256,
    ...
    return {'schema': 'flux-glyph-unified-region-font-v1', 'algorithm': 'unified-region-cnn64x256-v1',
        ...
        **copy.deepcopy(FIXED_RUNTIME), 'font_label_groups': cal['manifest']['font_label_groups'],
        ...
        'validation': {'kind': 'full_cnn_training_wenkai_recovery_calibration_only_at_export',
...
def export(args):
    ...
    require(not output.exists() and not (run/'PARITY.json').exists(), 'Preserve earlier WenKai-recovery export attempts')
    ...
    sources = {str((ROOT/name).resolve()): sha(ROOT/name) for name in
        ('training/export_unified_retention_wenkai_recovery.py', 'training/train_unified_retention_wenkai_recovery.py',
         'training/retention_wenkai_recovery_loss.py', 'training/train_unified_retention_margin.py',
         'training/retention_dual_teacher_loss.py', 'training/retention_confidence_floor_loss.py',
        'training/retention_angular_margin_loss.py', 'training/train_unified_retention_face_balanced.py',
