...
are never exposed as glyph boxes: every ``ok`` Han box has two image-derived
...
def segment_characters(image: Image.Image, text: str, tokens: list[dict[str, Any]], *, metadata: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    boxes. One record is returned for every character in ``text``.
    """
    ...
    for left_index, right_index in zip(valid_anchor_indices, valid_anchor_indices[1:]):
        ...
        if right_anchor[0] <= left_anchor[0]:
            continue
        cut, ok, evidence = _gap_boundary(mass, gaps, left_anchor, right_anchor, threshold, minimum_gap_width)
    ...
    if len(ink_columns):
        ...
        if 0 in anchors:
            ok = ink_left > 0
            edges[(0, "left")] = (ink_left if ok else None, ok, {
    ...
    for index, character in enumerate(text):
        ...
        if token == {}:
        ...
        elif not _han(character):
            item.update({"reason": "non_han_token_preserved_not_font_segmented", "ctc_anchor": list(anchors[index])})
        else:
            left_evidence = edges.get((index, "left")) if index == 0 else cuts.get(index - 1)
            right_evidence = edges.get((index, "right")) if index == len(text) - 1 else cuts.get(index)
            if left_evidence is None or right_evidence is None:
                ...
                ys = np.flatnonzero(foreground[:, max(0, left):min(width, right)].any(axis=1))
