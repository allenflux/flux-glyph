"""Independent Android-scope CNN; user-selected scope is not device evidence."""
...
SCHEMA = 'flux-glyph-android-region-font-v1'
ALGORITHM = 'android-region-cnn64x256-v1'
UNKNOWN = '__unknown__'
...
def android_metadata(metadata):
    ...
    if (not isinstance(metadata, dict) or metadata.get('schema') != SCHEMA
            or metadata.get('algorithm') != ALGORITHM or metadata.get('font_mode') != 'android'
            or metadata.get('data_kind') != 'android_emulator_screenshot'
            or 'rejection' in metadata or 'verifier' in metadata):
        raise ValueError('Invalid Android model contract')
    ...
    if (not isinstance(families, list) or len(families) != 10
            ...
            or len(set(families)) != 10 or families.count(UNKNOWN) != 1):
        raise ValueError('Android model must contain nine families and one unknown class')
    ...
    if (not _number(metadata.get('temperature'), .01, 100) or not isinstance(gates, dict)
        ...
        raise ValueError('Invalid Android model gates')
    ...
    if (not isinstance(groups, dict) or any(family not in public or not isinstance(names, list) or not names
        ...
        raise ValueError('Invalid Android font label groups')
    ...
    if (not isinstance(sources, dict) or any(family not in public or not isinstance(kinds, list) or not kinds
        ...
        raise ValueError('Invalid Android font sources')
    ...
    if (not isinstance(name, str) or Path(name).name != name or '\\' in name or not name.endswith('.onnx')
        ...
        raise ValueError('Invalid Android model path or SHA')
...
class AndroidFontClassifier:
    def __init__(self, directory):
        self.directory = Path(directory)
        path = self.directory/'metadata.json'
        if path.stat().st_size > 256*1024:
            raise ValueError('Android metadata exceeds size limit')
        self.meta = json.loads(path.read_text())
        validated = android_metadata(self.meta)
        ...
        if (not path.resolve().is_relative_to(self.directory.resolve()) or not path.is_file()
            ...
            raise ValueError('Android model missing or exceeds bounds')
        data = path.read_bytes()
        if hashlib.sha256(data).hexdigest() != validated['model']['sha256']:
            raise ValueError('Android model SHA differs')
        ...
        if (len(inputs) != 1 or inputs[0].name != 'tiles' or inputs[0].type != 'tensor(float)'
                ...
                or len(outputs[0].shape) != 2 or isinstance(outputs[0].shape[0], int) or outputs[0].shape[1] != 10
                or len(outputs[1].shape) != 1 or isinstance(outputs[1].shape[0], int)):
            raise ValueError('Android ONNX input/output contract differs')

    def predict(self, image):
        prepared = preprocess_region(image)
        result = {'method': 'region_neural_network', 'font_mode': 'android', 'status': 'uncertain', 'family': None,
        ...
        if (not isinstance(outputs, (list, tuple)) or len(outputs) != 2
                ...
                or outputs[0].shape != (len(prepared['tiles']), 10) or outputs[1].shape != (len(prepared['tiles']),)
        ...
        # Keep the original ten-class softmax. The unknown class is never
...
RegionFontClassifier = AndroidFontClassifier
