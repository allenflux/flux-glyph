...
def font_kit(engine):
    ...
    rejection=None if android else rejection_metadata(model.meta)
    ...
    verifier=None if android else verifier_metadata(model.meta)
    verifier_bytes=None
    ...
    if not android and metadata['font_label_groups']:
    ...
    files={name:weights,'metadata.json':(json.dumps(metadata,ensure_ascii=False,indent=2)+'\n').encode(),
           'inference.py':(folder/('android_font.py' if android else 'region_font.py')).read_bytes(),'text_style.py':(folder/'text_style.py').read_bytes(),
    ...
    if android:files['region_font.py']=(folder/'region_font.py').read_bytes()
    ...
    info={'available':True,'format':'ONNX','version':engine.version,'families':model.families,
          ...
          'input_shape':[None,1,64,256],'download_url':'/api/models/font/download'+('?mode=android' if android else ''),'bytes':len(data),
          'font_mode':'android' if android else 'ios',
          'sha256':hashlib.sha256(data).hexdigest(),'ocr_required':False,
          'unknown_font_rejection':android or rejection is not None,
